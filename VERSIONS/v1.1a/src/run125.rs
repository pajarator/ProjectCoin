/// RUN125 — Ease of Movement Confirmation: Volume/Price Efficiency Filter
///
/// Grid: EMV_LOOKBACK [10, 14, 20] × EMV_THRESH [0.1, 0.5, 1.0] (symmetric for LONG/SHORT)
/// 9 configs × 18 coins = 162 simulations (parallel per coin)
///
/// Run: cargo run --release --features run125 -- --run125
...
impl EmvCfg {
    fn label(&self) -> String {
        ...
        else { format!("LB{}_T{}", self.lookback, self.threshold) }
    }
}

fn build_grid() -> Vec<EmvCfg> {
    let mut grid = vec![EmvCfg { lookback: 0, threshold: 0.0, is_baseline: true }];
    for &lb in &[10usize, 14, 20] {
        for &th in &[0.1f64, 0.5, 1.0] {
            grid.push(EmvCfg { lookback: lb, threshold: th, is_baseline: false });
        }
    }
    grid
}
...
fn load_15m(coin: &str) -> Option<CoinData> {
    ...
    // Compute Ease of Movement: EMV = (midpoint_diff / box_ratio) smoothed
    // midpoint_diff = ((high + low)/2 - (prior_high + prior_low)/2)
    // box_ratio = (volume / (high - low)) / 10000
    // EMV_raw = midpoint_diff / box_ratio
    // EMV = SMA(EMV_raw, lookback)
    let lookback = 14;
    ...
    for i in 1..n {
        ...
        let range = high - low;
        let midpoint_diff = ((high + low) / 2.0) - ((prev_high + prev_low) / 2.0);
        let vol = volumes[i];
        if range > 1e-10 {
            let box_ratio = (vol / range) / 10000.0;
            if box_ratio > 1e-10 {
                emv_raw[i] = midpoint_diff / box_ratio;
            }
        }
    }
    ...
    for i in lookback..n {
        ...
        for j in 0..lookback {
            let idx = i - j;
            if !emv_raw[idx].is_nan() {
                sum += emv_raw[idx];
                count += 1;
            }
        }
        emv[i] = if count > 0 { sum / count as f64 } else { 0.0 };
    }

    Some(CoinData { name: coin.to_string(), opens, closes, highs, lows, volumes, zscore, emv })
}

fn simulate(d: &CoinData, cfg: &EmvCfg) -> CoinResult {
    let n = d.closes.len();
        ...
        if let Some((dir, entry, entry_bar)) = pos.as_mut() {
            ...
            if let Some(dir) = regime_signal(d.zscore[i]) {
                if cfg.is_baseline {
                    ...
                    let emv_val = d.emv[i];
                    let passes_filter = if emv_val.is_nan() {
                        true
                    } else if dir == 1 {
                        // LONG: require EMV > threshold (price moving up easily)
                        emv_val > cfg.threshold
                    } else {
                        // SHORT: require EMV < -threshold (price moving down easily)
                        emv_val < -cfg.threshold
                    };
...
pub fn run(shutdown: Arc<AtomicBool>) {
    eprintln!("RUN125 — Ease of Movement Confirmation Grid Search\n");
    ...
    println!("\n=== RUN125 Ease of Movement Confirmation Results ===");
    ...
    let notes = format!("RUN125 EMV filter. {} configs. Baseline PnL={:.2}. Best: {} (PnL={:.2}, Δ={:+.2})",
    ...
    std::fs::write("/home/scamarena/ProjectCoin/run125_1_results.json", &serde_json::to_string_pretty(&output).unwrap()).ok();
    eprintln!("\nSaved → run125_1_results.json");
}
