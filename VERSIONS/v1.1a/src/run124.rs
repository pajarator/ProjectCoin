/// RUN124 — Choppiness Index Confirmation: Confirm Market Is Choppy Before Mean-Reversion Entries
///
/// Grid: CI_LOOKBACK [10, 14, 20] × CI_CHOPPY_THRESH [40, 50, 60]
/// 9 configs × 18 coins = 162 simulations (parallel per coin)
///
/// Run: cargo run --release --features run124 -- --run124
...
impl CiCfg {
    fn label(&self) -> String {
        ...
        else { format!("LB{}_CT{}", self.lookback, self.choppy_thresh as usize) }
    }
}

fn build_grid() -> Vec<CiCfg> {
    let mut grid = vec![CiCfg { lookback: 0, choppy_thresh: 0.0, is_baseline: true }];
    for &lb in &[10usize, 14, 20] {
        for &ct in &[40.0f64, 50.0, 60.0] {
            grid.push(CiCfg { lookback: lb, choppy_thresh: ct, is_baseline: false });
        }
    }
    grid
}
...
fn load_15m(coin: &str) -> Option<CoinData> {
    ...
    for i in lookback..n {
        ...
        for j in 0..lookback {
            let idx = i - j;
            if highs[idx] > max_high { max_high = highs[idx]; }
            if lows[idx] < min_low { min_low = lows[idx]; }
        }
        let hl_range = max_high - min_low;
        if hl_range > 1e-10 && atr_sum > 0.0 {
            let log_period = (lookback as f64).log10();
            let ratio = atr_sum / hl_range;
            ci[i] = 100.0 * ratio.log10() / log_period;
        }
    }

    Some(CoinData { name: coin.to_string(), opens, closes, highs, lows, zscore, ci })
}

fn simulate(d: &CoinData, cfg: &CiCfg) -> CoinResult {
    let n = d.closes.len();
        ...
        if let Some((dir, entry, entry_bar)) = pos.as_mut() {
            ...
            if let Some(dir) = regime_signal(d.zscore[i]) {
                if cfg.is_baseline {
                    ...
                    // CI must be above choppy threshold for mean-reversion to work
                    let ci_val = d.ci[i];
                    let passes_filter = if ci_val.is_nan() {
                        true
                    } else {
                        ci_val > cfg.choppy_thresh
                    };
...
pub fn run(shutdown: Arc<AtomicBool>) {
    eprintln!("RUN124 — Choppiness Index Confirmation Grid Search\n");
    ...
    println!("\n=== RUN124 Choppiness Index Confirmation Results ===");
    ...
    let notes = format!("RUN124 Choppiness Index filter. {} configs. Baseline PnL={:.2}. Best: {} (PnL={:.2}, Δ={:+.2})",
    ...
    std::fs::write("/home/scamarena/ProjectCoin/run124_1_results.json", &serde_json::to_string_pretty(&output).unwrap()).ok();
    eprintln!("\nSaved → run124_1_results.json");
}
