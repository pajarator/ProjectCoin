...
///
/// Grid: CONFIDENCE_MULT [0.25, 0.5, 0.75, 1.0] × Z_ENTRY_MAX [2.5, 3.0, 3.5]
///
/// Run: cargo run --release --features run52 -- --run52
...
impl ZPosCfg {
    fn label(&self) -> String {
        if self.conf_mult == 0.0 { "DISABLED".to_string() }
        else { format!("CM{:.2}_ZM{:.1}", self.conf_mult, self.z_entry_max) }
    }
}

fn build_grid() -> Vec<ZPosCfg> {
    let mut grid = vec![ZPosCfg { conf_mult: 0.0, z_entry_max: 3.0 }]; // baseline
    let mult_vals = [0.25, 0.5, 0.75, 1.0];
    let zmax_vals = [2.5, 3.0, 3.5];
    for &m in &mult_vals {
        for &z in &zmax_vals {
            grid.push(ZPosCfg { conf_mult: m, z_entry_max: z });
        }
    }
    grid
}
...
fn confidence_position_fraction(z: f64, cfg: ZPosCfg) -> f64 {
    if cfg.conf_mult == 0.0 { return BASE_POSITION_SIZE; }
    let z_abs = z.abs();
    if z_abs < 1.5 { return BASE_POSITION_SIZE; }
    let z_entry_min = 1.5;
    let confidence = ((z_abs - z_entry_min) / (cfg.z_entry_max - z_entry_min)).min(1.0).max(0.0);
    BASE_POSITION_SIZE * (1.0 + cfg.conf_mult * confidence)
}

fn simulate(d: &CoinData15m, cfg: ZPosCfg) -> (f64, usize, usize, usize) {
    let n = d.closes.len();
    ...
    for i in 1..n {
        if let Some((dir, entry)) = pos {
            ...
            if closed {
                let pos_frac = confidence_position_fraction(d.zscore[i], cfg);
                let net = (bal * pos_frac * LEVERAGE) * exit_pct;
                bal += net;
...
pub fn run(shutdown: Arc<AtomicBool>) {
    eprintln!("RUN52 — Z-Score Deviation Position Sizing\n");
    ...
    let results: Vec<ConfigResult> = grid.par_iter().map(|cfg| {
        if shutdown.load(Ordering::SeqCst) {
            return ConfigResult { label: cfg.label(), total_pnl: 0.0, portfolio_wr: 0.0, total_trades: 0, pf: 0.0, is_baseline: cfg.conf_mult == 0.0 };
        }
        ...
        let is_baseline = cfg.conf_mult == 0.0;
    ...
    println!("\n=== RUN52 Z-Score Position Sizing Results ===");
    ...
    let notes = format!("RUN52 z-score position sizing. {} configs. Baseline PnL={:.2}. Best: {} (PnL={:.2}, Δ={:+.2})",
    ...
    std::fs::write("/home/scamarena/ProjectCoin/run52_1_results.json", &serde_json::to_string_pretty(&output).unwrap()).ok();
    eprintln!("\nSaved → run52_1_results.json");
}
