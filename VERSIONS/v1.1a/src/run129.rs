/// RUN129 — VWAP Deviation Percentile Filter: Require Extreme VWAP Distance
///
/// Grid: VWAP_DEV_WINDOW [50, 100, 200] × LONG_PCT [5, 10, 15] (symmetric SHORT)
/// 9 configs × 18 coins = 162 simulations (parallel per coin)
///
/// Run: cargo run --release --features run129 -- --run129
...
impl VwapCfg {
    fn label(&self) -> String {
        ...
        else { format!("W{}_LP{}", self.window, self.long_pct as usize) }
    }
}

fn build_grid() -> Vec<VwapCfg> {
    let mut grid = vec![VwapCfg { window: 0, long_pct: 0.0, is_baseline: true }];
    for &w in &[50usize, 100, 200] {
        for &lp in &[5.0f64, 10.0, 15.0] {
            grid.push(VwapCfg { window: w, long_pct: lp, is_baseline: false });
        }
    }
    grid
}
...
fn compute_vwap(highs: &[f64], lows: &[f64], closes: &[f64], volumes: &[f64], period: usize) -> Vec<f64> {
    let n = closes.len();
...
fn percentile_rank(data: &[f64], current: f64) -> f64 {
    ...
    let more_extreme = data.iter().filter(|&&x| {
        if current < 0.0 { x <= current } else { x >= current }
    }).count();
...
fn load_15m(coin: &str) -> Option<CoinData> {
    ...
    let vwap = compute_vwap(&highs, &lows, &closes, &volumes, 20);

    let mut vwap_dev_pct = vec![f64::NAN; n];
    for i in 0..n {
        if !vwap[i].is_nan() && vwap[i] > 0.0 {
            vwap_dev_pct[i] = (closes[i] - vwap[i]) / vwap[i];
        }
    }

    Some(CoinData { name: coin.to_string(), opens, closes, highs, lows, volumes, zscore, vwap, vwap_dev_pct })
}

fn simulate(d: &CoinData, cfg: &VwapCfg) -> CoinResult {
    let n = d.closes.len();
        ...
        if let Some((dir, entry, entry_bar)) = pos.as_mut() {
            ...
            if let Some(dir) = regime_signal(d.zscore[i]) {
                if cfg.is_baseline {
                    ...
                    // Compute percentile rank of VWAP deviation
                    let window = cfg.window;
                    let start = if i >= window { i - window } else { 0 };
                    let history = &d.vwap_dev_pct[start..i];
                    let current_dev = d.vwap_dev_pct[i];
                    let pct_rank = if current_dev.is_nan() { 50.0 } else { percentile_rank(history, current_dev) };

                    let short_pct = cfg.long_pct; // symmetric
                    let passes_filter = if dir == 1 {
                        // LONG: percentile rank must be <= long_pct (most extreme negative)
                        pct_rank <= cfg.long_pct
                    } else {
                        // SHORT: percentile rank must be >= (100 - short_pct) (most extreme positive)
                        pct_rank >= (100.0 - short_pct)
                    };
...
pub fn run(shutdown: Arc<AtomicBool>) {
    eprintln!("RUN129 — VWAP Deviation Percentile Filter Grid Search\n");
    ...
    println!("\n=== RUN129 VWAP Deviation Percentile Filter Results ===");
    ...
    let notes = format!("RUN129 VWAP Deviation filter. {} configs. Baseline PnL={:.2}. Best: {} (PnL={:.2}, Δ={:+.2})",
    ...
    std::fs::write("/home/scamarena/ProjectCoin/run129_1_results.json", &serde_json::to_string_pretty(&output).unwrap()).ok();
    eprintln!("\nSaved → run129_1_results.json");
}
