/// RUN32 — Regime-only day-by-day backtest (COINCLAW v15 regime layer)
///
/// Simulates the per-coin 15m regime strategies across a full year of data.
/// Scalp overlay, momentum, and complement strategies are EXCLUDED from this run
/// so you can see the pure regime signal P&L in isolation.
///
/// Each calendar day starts at $100/coin ($1800 total). Positions not closed by
/// midnight are force-closed at the last bar's close. This matches the run30 methodology.
///
/// Since the regime config is IDENTICAL across v9 through v15 (only the scalp overlay
/// changed between versions), this produces a single result column.
///
/// Run: cargo run --release --features run32 -- --run32
...
const MIN_HOLD: usize  = 2;       // min candles before signal exit
const FEE: f64         = 0.001;   // 0.10% per side
const SLIP: f64        = 0.0005;  // 0.05% per side
const RISK: f64        = 0.10;    // 10% of balance per trade (RISK constant)
const LEVERAGE: f64    = 5.0;     // 5×
const INITIAL_BAL: f64 = 100.0;   // $100/coin/day

// Breadth thresholds
const BREADTH_LONG_MAX:  f64 = 0.20;
const BREADTH_SHORT_MIN: f64 = 0.50;

// ISO short thresholds (matching config.rs)
const ISO_Z_THRESHOLD: f64   = 1.5;
const ISO_BB_MARGIN:   f64   = 0.98;
const ISO_VOL_MULT:    f64   = 1.2;
const ISO_ADR_PCT:     f64   = 0.25;
...
struct Ind {
    ...
    bb_lo:        f64,
    bb_hi:        f64,
    bb_width:     f64,  // 4 * std20 (matching live)
    bb_width_avg: f64,  // 20-bar avg of bb_width
    vol:          f64,
    vol_ma:       f64,
    adr_lo:       f64,  // rolling_min(lows, 24 bars) ≈ 6h
    adr_hi:       f64,  // rolling_max(highs, 24 bars) ≈ 6h
    rsi14:        f64,
    rsi7:         f64,
    vwap:         f64,  // 20-bar rolling TPVOL/TVOL
    adx:          f64,
    ou_halflife:  f64,
    ou_deviation: f64,
    // Laguerre RSI (gamma = 0.5/0.6/0.7/0.8)
...
// Laguerre RSI — causal recursive filter, returns (current, prev) per bar
...
// Kalman filter — forward pass, returns (estimate, variance) per bar
...
// KST — returns (kst, signal, kst_prev, sig_prev) per bar
...
// OU half-life at bar i (rolling OU_WINDOW-bar regression)
...
fn compute_indicators(bars: &[Bar], is_dash: bool) -> Vec<Ind> {
    ...
    // Complement indicators (all coins — we use per-coin cfg to decide which to apply)
...
fn long_signal(ind: &Ind, strat: LongStrat) -> bool {
    ...
    // Block if price > sma20 or z > 0.5 (matching live guard: ind.p > ind.sma20 || ind.z > 0.5)
    ...
    match strat {
        ...
        LongStrat::BbBounce => {
            !ind.bb_lo.is_nan() && ind.vol > ind.vol_ma * 1.3
                && p <= ind.bb_lo * 1.02
        }
        LongStrat::AdrRev => {
            !ind.adr_lo.is_nan() && !ind.adr_hi.is_nan() && {
                let range = ind.adr_hi - ind.adr_lo;
                range > 0.0 && p <= ind.adr_lo + range * 0.25
    ...
    match strat {
        ...
        ShortStrat::AdrRev => {
            !ind.adr_hi.is_nan() && !ind.adr_lo.is_nan() && {
                let range = ind.adr_hi - ind.adr_lo;
                range > 0.0 && p >= ind.adr_hi - range * 0.25
                    && ind.vol > ind.vol_ma * 1.1
            }
        }
    }
...
fn simulate(
    ...
    // BTC index for btc_z in iso context
    ...
    for bar_i in 1..n_bars {
        ...
        if bar_day != current_day {
            for ci in 0..N_COINS {
                if let Some(ref pos) = positions[ci] {
                    ...
                    *daily_pnl.entry(current_day).or_insert(0.0) += pf * notional;
                    total_trades += 1;
        ...
        // ── Compute cross-coin context for this bar ──────────────────────────
        ...
        for ci in 0..N_COINS {
            if let Some(ref pos) = positions[ci] {
                ...
                if pnl <= -SL {
                    ...
                    // Signal exit: price crossed back through SMA20 or Z reverted
...
pub fn run(shutdown: Arc<AtomicBool>) {
    eprintln!("RUN32 — Regime-only day-by-day backtest (v15 regime layer)");
    eprintln!("NOTE: Regime config is identical across v9–v15. Single result column.");
    eprintln!("      Scalp overlay EXCLUDED. Complement strategies INCLUDED.");
    ...
    if last_month {
        ...
        ordered_days = ordered_days[ordered_days.len()-keep..].to_vec();
    ...
    eprintln!("Simulating regime layer...");
    let (daily, trades, wins) = simulate(&all_inds, &all_bars, &all_days, &ordered_days);
    let wr = if trades > 0 { wins as f64 / trades as f64 * 100.0 } else { 0.0 };
    eprintln!("Done — {} trades, {:.1}% WR", trades, wr);

    // ── Print table ───────────────────────────────────────────────────────────
    let mut daily_rows: Vec<DailyRow> = Vec::new();
    for &day in &ordered_days {
        let y = day / 10000; let m = (day % 10000) / 100; let d = day % 100;
        daily_rows.push(DailyRow {
            date:   format!("{:04}-{:02}-{:02}", y, m, d),
            regime: daily.get(&day).cloned().unwrap_or(0.0),
        });
    }
    ...
    for row in &daily_rows {
        println!("{:<12} {:>+12.3}", row.date, row.regime);
    }
    println!("{}", "=".repeat(26));

    let pnls: Vec<f64> = ordered_days.iter().map(|d| daily.get(d).cloned().unwrap_or(0.0)).collect();
    ...
    println!("Total: ${:+.2}  avg/day: ${:+.4}  best: ${:+.3}  worst: ${:+.3}", total, avg, best, worst);
    println!("+days: {}  -days: {}  ~days: {}  trades: {}  WR: {:.1}%",
        pos, neg, pnls.len()-pos-neg, trades, wr);

    let notes = format!(
        "Regime-only backtest. {} coins, {} days. \
         $100/coin/day reset. RISK={}% LEVERAGE={}x FEE={}%/side SLIP={}%/side. \
         Complement strategies INCLUDED. Scalp/Momentum EXCLUDED. \
         DASH OU halflife computed. v9-v15 regime config identical.",
        N_COINS, ordered_days.len(), (RISK*100.0) as u32, LEVERAGE as u32,
        FEE*100.0, SLIP*100.0
    );
    let output = Output {
        notes, daily: daily_rows,
        summary: Summary {
            total_pnl: total, avg_daily_pnl: avg,
            best_day: best, worst_day: worst,
            positive_days: pos, negative_days: neg,
            total_days: pnls.len(), total_trades: trades,
            win_rate_pct: wr,
        },
    };
    let json = serde_json::to_string_pretty(&output).unwrap();
    let path = "/home/scamarena/ProjectCoin/run32_1_results.json";
