/// RUN109 — Minimum Volume Surge Confirmation: Require Volume Spike at Entry
///
/// Grid: SURGE_LONG [1.5, 2.0, 2.5, 3.0] × SURGE_SHORT [1.5, 2.0, 2.5, 3.0]
/// 16 configs × 18 coins = 288 simulations (parallel per coin)
///
/// Run: cargo run --release --features run109 -- --run109
...
impl VolSurgeCfg {
    fn label(&self) -> String {
        ...
        else { format!("SL{:.1}_SS{:.1}", self.surge_long, self.surge_short) }
    }
}

fn build_grid() -> Vec<VolSurgeCfg> {
    let mut grid = vec![VolSurgeCfg { surge_long: 0.0, surge_short: 0.0, is_baseline: true }];
    for &sl in &[1.5, 2.0, 2.5, 3.0] {
        for &ss in &[1.5, 2.0, 2.5, 3.0] {
            grid.push(VolSurgeCfg { surge_long: sl, surge_short: ss, is_baseline: false });
        }
    }
    grid
}
...
fn load_15m(coin: &str) -> Option<CoinData> {
    ...
    let mut highs = Vec::new(); let mut lows = Vec::new(); let mut vol = Vec::new();
...
fn simulate(d: &CoinData, cfg: &VolSurgeCfg) -> CoinResult {
    let n = d.closes.len();
        ...
        if let Some((dir, entry, entry_bar, _z_entry)) = pos.as_mut() {
            ...
            if let Some(dir) = regime_signal(d.zscore[i]) {
                ...
                if cfg.is_baseline {
                    ...
                    let vol_ratio = if d.vol_ma[i] > 0.0 { d.vol[i] / d.vol_ma[i] } else { 999.0 };
                    let passes_filter = if dir == 1 {
                        vol_ratio >= cfg.surge_long
                    } else {
                        vol_ratio >= cfg.surge_short
                    };
                    if passes_filter {
                        if i + 1 < n {
                            let entry_price = d.opens[i + 1];
...
pub fn run(shutdown: Arc<AtomicBool>) {
    eprintln!("RUN109 — Minimum Volume Surge Confirmation Grid Search\n");
    ...
    println!("\n=== RUN109 Minimum Volume Surge Confirmation Results ===");
    ...
    let notes = format!("RUN109 min volume surge. {} configs. Baseline PnL={:.2}. Best: {} (PnL={:.2}, Δ={:+.2}, filter_rate={:.1}%)",
    ...
    std::fs::write("/home/scamarena/ProjectCoin/run109_1_results.json", &serde_json::to_string_pretty(&output).unwrap()).ok();
    eprintln!("\nSaved → run109_1_results.json");
}
