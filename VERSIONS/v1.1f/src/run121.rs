/// RUN121 — TD Sequential Entry Filter: Countdown-Based Entry Confirmation
///
/// Grid: TD_SETUP_LEN [9, 13] × TD_SETUP_LOOKBACK [4, 6, 8] × TD_STRICT [true, false]
/// 12 configs × 18 coins = 216 simulations (parallel per coin)
///
/// Run: cargo run --release --features run121 -- --run121
...
impl TdCfg {
    fn label(&self) -> String {
        if self.is_baseline { "BASELINE".to_string() }
            ...
            let s = if self.strict { "S" } else { "NS" };
            format!("SL{}_LB{}_{}", self.setup_len, self.lookback, s)
        }
    }
}

fn build_grid() -> Vec<TdCfg> {
    let mut grid = vec![TdCfg { setup_len: 0, lookback: 0, strict: false, is_baseline: true }];
    for &sl in &[9usize, 13] {
        for &lb in &[4usize, 6, 8] {
            for &st in &[true, false] {
                grid.push(TdCfg { setup_len: sl, lookback: lb, strict: st, is_baseline: false });
            }
        }
    }
    grid
...
fn simulate(d: &CoinData, cfg: &TdCfg) -> CoinResult {
    let n = d.closes.len();
    ...
    for i in 20..n {
        ...
        // Check TD Sequential setup
        let has_td_setup = |dir: i8, idx: usize| -> bool {
            if idx < cfg.setup_len + cfg.lookback || idx >= n { return false; }
            let mut count = 0usize;
            for k in 0..cfg.setup_len {
                let c = d.closes[idx - k];
                let c_prev = d.closes[idx - k - 4];
                let ok = if dir == 1 { c > c_prev } else { c < c_prev };
                if ok { count += 1; } else { break; }
            }
            if cfg.strict { count == cfg.setup_len } else { count >= cfg.setup_len }
        };
        ...
        if let Some((dir, entry, entry_bar)) = pos.as_mut() {
            ...
            if let Some(dir) = regime_signal(d.zscore[i]) {
                if cfg.is_baseline {
                    ...
                    // Check if TD setup completed within lookback bars
                    let mut has_setup = false;
                    for lookback in 1..=cfg.lookback.min(i) {
                        if has_td_setup(dir, i - lookback) {
                            has_setup = true;
                            break;
                        }
                    }
...
pub fn run(shutdown: Arc<AtomicBool>) {
    eprintln!("RUN121 — TD Sequential Entry Filter Grid Search\n");
    ...
    println!("\n=== RUN121 TD Sequential Entry Filter Results ===");
    ...
    let notes = format!("RUN121 TD Sequential filter. {} configs. Baseline PnL={:.2}. Best: {} (PnL={:.2}, Δ={:+.2})",
    ...
    std::fs::write("/home/scamarena/ProjectCoin/run121_1_results.json", &serde_json::to_string_pretty(&output).unwrap()).ok();
    eprintln!("\nSaved → run121_1_results.json");
}
