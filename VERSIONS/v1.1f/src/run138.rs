/// RUN138 — Opening Range Breakout Filter: Require Price to Be Within Recent Range for Mean-Reversion
///
/// Grid: ORB_BARS [3, 5, 7] × ORB_THRESH [0.0005, 0.001, 0.002]
/// 9 configs × 18 coins = 162 simulations (parallel per coin)
///
/// Run: cargo run --release --features run138 -- --run138
...
impl OrbCfg {
    fn label(&self) -> String {
        ...
        else { format!("B{}_T{}", self.bars, (self.thresh * 10000.0) as usize) }
    }
}

fn build_grid() -> Vec<OrbCfg> {
    let mut grid = vec![OrbCfg { bars: 0, thresh: 0.0, is_baseline: true }];
    for &b in &[3usize, 5, 7] {
        for &t in &[0.0005f64, 0.001, 0.002] {
            grid.push(OrbCfg { bars: b, thresh: t, is_baseline: false });
        }
    }
    grid
}
...
fn simulate(d: &CoinData, cfg: &OrbCfg) -> CoinResult {
    let n = d.closes.len();
        ...
        if let Some((dir, entry, entry_bar)) = pos.as_mut() {
            ...
            if let Some(dir) = regime_signal(d.zscore[i]) {
                if cfg.is_baseline {
                    ...
                    // ORB filter: price must be within the recent range + threshold buffer
                    // Find max high and min low over last ORB_BARS
                    let start = i.saturating_sub(cfg.bars);
                    let mut max_high = f64::MIN;
                    let mut min_low = f64::MAX;
                    for j in start..=i {
                        if d.highs[j] > max_high { max_high = d.highs[j]; }
                        if d.lows[j] < min_low { min_low = d.lows[j]; }
                    }
                    let range_mid = (max_high + min_low) / 2.0;
                    let current = d.closes[i];

                    // Filter: price must be within range + threshold buffer
                    // For LONG: price must be near the low end (within thresh of min_low)
                    // For SHORT: price must be near the high end (within thresh of max_high)
                    let passes_filter = if dir == 1 {
                        // LONG: current price must be within thresh of min_low
                        let buffer = (max_high - min_low) * cfg.thresh;
                        current <= min_low + buffer
                    } else {
                        // SHORT: current price must be within thresh of max_high
                        let buffer = (max_high - min_low) * cfg.thresh;
                        current >= max_high - buffer
                    };
...
pub fn run(shutdown: Arc<AtomicBool>) {
    eprintln!("RUN138 — Opening Range Breakout Filter Grid Search\n");
    ...
    println!("\n=== RUN138 Opening Range Breakout Filter Results ===");
    ...
    let notes = format!("RUN138 ORB filter. {} configs. Baseline PnL={:.2}. Best: {} (PnL={:.2}, Δ={:+.2})",
    ...
    std::fs::write("/home/scamarena/ProjectCoin/run138_1_results.json", &serde_json::to_string_pretty(&output).unwrap()).ok();
    eprintln!("\nSaved → run138_1_results.json");
}
