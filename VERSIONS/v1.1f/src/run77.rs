...
///
/// Exit regime trades when mean reversion is stalling: if held > MIN_BARS and
/// recovery_velocity < MIN_VELOCITY, force close.
///
/// Grid: MIN_BARS [15, 20, 30] × MIN_VELOCITY [0.010, 0.015, 0.020, 0.025] × GRACE [5, 10, 15]
/// Total: 3 × 4 × 3 = 36 + baseline = 37 configs
///
/// Run: cargo run --release --features run77 -- --run77
...
impl RecoveryCfg {
    fn label(&self) -> String {
        ...
        else { format!("MB{}_MV{:.3}_GR{}", self.min_bars, self.min_velocity, self.grace_bars) }
    }
}

fn build_grid() -> Vec<RecoveryCfg> {
    let mut grid = vec![RecoveryCfg { min_bars: 0, min_velocity: 0.0, grace_bars: 0, is_baseline: true }];
    let min_bars_list = [15, 20, 30];
    let min_vels = [0.010, 0.015, 0.020, 0.025];
    let graces = [5, 10, 15];
    for &mb in &min_bars_list {
        for &mv in &min_vels {
            for &gr in &graces {
                grid.push(RecoveryCfg { min_bars: mb, min_velocity: mv, grace_bars: gr, is_baseline: false });
            }
        }
    }
    grid
...
#[derive(Serialize)]
struct CoinResult { coin: String, pnl: f64, trades: usize, wins: usize, losses: usize, wr: f64, recovery_exits: usize }
#[derive(Serialize)]
struct ConfigResult {
    label: String, total_pnl: f64, portfolio_wr: f64, total_trades: usize,
    pf: f64, is_baseline: bool, recovery_exit_rate: f64, coins: Vec<CoinResult>,
}
#[derive(Serialize)]
...
fn simulate(d: &CoinData15m, cfg: &RecoveryCfg) -> (f64, usize, usize, usize, usize, usize) {
    let n = d.closes.len();
    ...
    for i in 1..n {
        if let Some((dir, entry, z_entry, entry_bar)) = pos {
            ...
            // Recovery rate exit (only in non-baseline)
            if !closed && !cfg.is_baseline && held > cfg.min_bars {
                let effective_held = held.saturating_sub(cfg.grace_bars);
                if effective_held > 0 {
                    let z_curr = d.zscore[i];
                    if !z_curr.is_nan() {
                        // recovery_velocity = (z_entry - z_curr) / effective_held
                        // positive = recovering (z moving toward 0)
                        let recovery_vel = if dir == 1 {
                            // Long: z_entry < 0, want z to increase toward 0
                            (z_curr - z_entry) / effective_held as f64
                        } else {
                            // Short: z_entry > 0, want z to decrease toward 0
                            (z_entry - z_curr) / effective_held as f64
                        };
                        if recovery_vel < cfg.min_velocity {
                            exit_pct = pct;
                            closed = true;
                            recovery_exits += 1;
                        }
    ...
    (bal - INITIAL_BAL, wins, losses, flats, recovery_exits, 0)
}

pub fn run(shutdown: Arc<AtomicBool>) {
    eprintln!("RUN77 — Z-Score Recovery Rate Exit\n");
    ...
    let results: Vec<ConfigResult> = grid.par_iter().map(|cfg| {
        if shutdown.load(Ordering::SeqCst) {
            return ConfigResult {
                ...
                pf: 0.0, is_baseline: cfg.is_baseline, recovery_exit_rate: 0.0, coins: vec![],
            };
        }
        let coin_results: Vec<CoinResult> = coin_data.iter().map(|d| {
            let (pnl, wins, losses, flats, recovery_exits, _) = simulate(d, cfg);
            let trades = wins + losses + flats;
            let wr = if trades > 0 { wins as f64 / trades as f64 * 100.0 } else { 0.0 };
            CoinResult { coin: d.name.clone(), pnl, trades, wins, losses, wr, recovery_exits }
        }).collect();
        ...
        let total_recovery: usize = coin_results.iter().map(|c| c.recovery_exits).sum();
        ...
        let recovery_rate = if total_trades > 0 { total_recovery as f64 / total_trades as f64 * 100.0 } else { 0.0 };

        let d = done.fetch_add(1, Ordering::SeqCst) + 1;
        eprintln!("  [{:>3}/{}] {}  PnL={:>+8.2}  WR={:>5.1}%  trades={}  rec_exit={:.1}%",
            d, total_cfgs, cfg.label(), total_pnl, portfolio_wr, total_trades, recovery_rate);

        ConfigResult {
            label: cfg.label(), total_pnl, portfolio_wr, total_trades,
            pf, is_baseline: cfg.is_baseline, recovery_exit_rate: recovery_rate,
            coins: coin_results,
        }
    }).collect();
    ...
    println!("\n=== RUN77 Z-Score Recovery Rate Exit Results ===");
    println!("Baseline: PnL={:+.2}  WR={:.1}%  Trades={}", baseline.total_pnl, baseline.portfolio_wr, baseline.total_trades);
    println!("\n{:>3}  {:<22} {:>8} {:>8} {:>6} {:>7} {:>8} {:>9}",
        "#", "Config", "PnL", "ΔPnL", "WR%", "Trades", "PF", "RecEx%");
    println!("{}", "-".repeat(80));
    for (i, r) in sorted.iter().enumerate() {
        let delta = r.total_pnl - baseline.total_pnl;
        println!("{:>3}  {:<22} {:>+8.2} {:>+8.2} {:>5.1}%  {:>6} {:>8.2} {:>8.1}%",
            i+1, r.label, r.total_pnl, delta, r.portfolio_wr, r.total_trades, r.pf, r.recovery_exit_rate);
    }
    println!("{}", "=".repeat(80));
    ...
    let notes = format!("RUN77 recovery rate exit. {} configs. Baseline PnL={:.2}. Best: {} (PnL={:.2}, Δ={:+.2})",
    ...
    std::fs::write("/home/scamarena/ProjectCoin/run77_1_results.json", &serde_json::to_string_pretty(&output).unwrap()).ok();
    eprintln!("\nSaved → run77_1_results.json");
}
