/// RUN112 — Money Flow Index Confirmation: Require MFI Extreme at Entry
///
/// Grid: MFI_LONG_MAX [20, 30, 40] × MFI_SHORT_MIN [60, 70, 80]
/// 9 configs × 18 coins = 162 simulations (parallel per coin)
///
/// Run: cargo run --release --features run112 -- --run112
...
impl MfiCfg {
    fn label(&self) -> String {
        ...
        else { format!("LM{}_SM{}", self.mfi_long_max as usize, self.mfi_short_min as usize) }
    }
}

fn build_grid() -> Vec<MfiCfg> {
    let mut grid = vec![MfiCfg { mfi_long_max: 0.0, mfi_short_min: 0.0, is_baseline: true }];
    for &lm in &[20.0, 30.0, 40.0] {
        for &sm in &[60.0, 70.0, 80.0] {
            grid.push(MfiCfg { mfi_long_max: lm, mfi_short_min: sm, is_baseline: false });
        }
    }
    grid
}
...
fn compute_mfi(highs: &[f64], lows: &[f64], closes: &[f64], volumes: &[f64], period: usize) -> Vec<f64> {
    let n = closes.len();
    ...
    // Compute initial sum over first period
    for i in 1..period {
        let tp_prev = (highs[i-1] + lows[i-1] + closes[i-1]) / 3.0;
        let tp_curr = (highs[i] + lows[i] + closes[i]) / 3.0;
        let mf = tp_curr * volumes[i];
        if tp_curr > tp_prev {
            positive_flow += mf;
    ...
    for i in period..n {
        ...
        // Update flows: remove oldest, add newest
        let tp_oldest = (highs[i-period] + lows[i-period] + closes[i-period]) / 3.0;
        let mf_oldest = tp_oldest * volumes[i-period];

        if tp_prev > tp_oldest {
            positive_flow -= mf_oldest;
        } else if tp_prev < tp_oldest {
            negative_flow -= mf_oldest;
        }
        ...
        if negative_flow > 1e-10 {
            mfi[i] = 100.0 - (100.0 / (1.0 + positive_flow / negative_flow));
        } else {
            mfi[i] = 100.0;
        }
    }
...
fn load_15m(coin: &str) -> Option<CoinData> {
    ...
    let mfi = compute_mfi(&highs, &lows, &closes, &volumes, 14);

    Some(CoinData { name: coin.to_string(), opens, closes, highs, lows, zscore, mfi })
}
...
fn simulate(d: &CoinData, cfg: &MfiCfg) -> CoinResult {
    let n = d.closes.len();
        ...
        if let Some((dir, entry, entry_bar)) = pos.as_mut() {
            ...
            if let Some(dir) = regime_signal(d.zscore[i]) {
                if cfg.is_baseline {
                    ...
                    let mfi_val = d.mfi[i];
                    let passes_filter = if !mfi_val.is_nan() {
                        if dir == 1 { mfi_val < cfg.mfi_long_max }
                        else { mfi_val > cfg.mfi_short_min }
                    } else { true };
...
pub fn run(shutdown: Arc<AtomicBool>) {
    eprintln!("RUN112 — Money Flow Index Confirmation Grid Search\n");
    ...
    println!("\n=== RUN112 Money Flow Index Confirmation Results ===");
    ...
    for (i, r) in sorted.iter().enumerate().take(15) {
        ...
        println!("{:>3}  {:<16} {:>+8.2} {:>+8.2} {:>5.1}%  {:>6} {:>8} {:>7.1}%",
    ...
    let notes = format!("RUN112 MFI confirmation. {} configs. Baseline PnL={:.2}. Best: {} (PnL={:.2}, Δ={:+.2}, filter_rate={:.1}%)",
    ...
    std::fs::write("/home/scamarena/ProjectCoin/run112_1_results.json", &serde_json::to_string_pretty(&output).unwrap()).ok();
    eprintln!("\nSaved → run112_1_results.json");
}
