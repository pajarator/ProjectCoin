/// RUN114 — Aroon Regime Confirmation: Use Aroon to Confirm Range-Bound State at Entry
///
/// Grid: AROON_LOOKBACK [14, 25, 50] × AROON_OSC_MIN [10, 20, 30] × AROON_TREND_MAX [60, 70, 80]
/// 27 configs × 18 coins = 486 simulations (parallel per coin)
///
/// Run: cargo run --release --features run114 -- --run114
...
impl AroonCfg {
    fn label(&self) -> String {
        ...
        else { format!("LB{}_OM{}_TM{}", self.lookback, self.osc_min as usize, self.trend_max as usize) }
    }
}

fn build_grid() -> Vec<AroonCfg> {
    let mut grid = vec![AroonCfg { lookback: 0, osc_min: 0.0, trend_max: 0.0, is_baseline: true }];
    for &lb in &[14usize, 25, 50] {
        for &om in &[10.0, 20.0, 30.0] {
            for &tm in &[60.0, 70.0, 80.0] {
                grid.push(AroonCfg { lookback: lb, osc_min: om, trend_max: tm, is_baseline: false });
            }
        }
    }
    grid
...
fn compute_aroon(highs: &[f64], lows: &[f64], lookback: usize) -> (Vec<f64>, Vec<f64>) {
    let n = highs.len();
    let mut aroon_up = vec![f64::NAN; n];
    let mut aroon_down = vec![f64::NAN; n];

    for i in lookback..n {
        // Find bars since last high in window
        let window_high = highs[i+1-lookback..=i].iter().fold(f64::NEG_INFINITY, |a, &b| a.max(b));
        let bars_since_high = lookback - highs[i+1-lookback..=i].iter().position(|&x| (x - window_high).abs() < 1e-10).unwrap_or(lookback);
        aroon_up[i] = ((lookback - bars_since_high) as f64 / lookback as f64) * 100.0;

        let window_low = lows[i+1-lookback..=i].iter().fold(f64::INFINITY, |a, &b| a.min(b));
        let bars_since_low = lookback - lows[i+1-lookback..=i].iter().position(|&x| (x - window_low).abs() < 1e-10).unwrap_or(lookback);
        aroon_down[i] = ((lookback - bars_since_low) as f64 / lookback as f64) * 100.0;
    }
    (aroon_up, aroon_down)
}
...
fn load_15m(coin: &str) -> Option<CoinData> {
    ...
    // Use default lookback of 25 for data storage
    let (aroon_up, aroon_down) = compute_aroon(&highs, &lows, 25);

    Some(CoinData { name: coin.to_string(), opens, closes, highs, lows, zscore, aroon_up, aroon_down })
}
...
fn simulate(d: &CoinData, cfg: &AroonCfg) -> CoinResult {
    let n = d.closes.len();
        ...
        if let Some((dir, entry, entry_bar)) = pos.as_mut() {
            ...
            if let Some(dir) = regime_signal(d.zscore[i]) {
                if cfg.is_baseline {
                    ...
                    let aroon_up_val = d.aroon_up[i];
                    let aroon_down_val = d.aroon_down[i];
                    let passes_filter = if !aroon_up_val.is_nan() && !aroon_down_val.is_nan() {
                        let osc = aroon_down_val - aroon_up_val;
                        if dir == 1 {
                            // LONG: Aroon Down above Aroon Up by osc_min, neither in strong trend
                            osc >= cfg.osc_min
                                && aroon_up_val < cfg.trend_max
                                && aroon_down_val < cfg.trend_max
                        } else {
                            // SHORT: Aroon Up above Aroon Down by osc_min, neither in strong trend
                            -osc >= cfg.osc_min
                                && aroon_up_val < cfg.trend_max
                                && aroon_down_val < cfg.trend_max
                        }
...
pub fn run(shutdown: Arc<AtomicBool>) {
    eprintln!("RUN114 — Aroon Regime Confirmation Grid Search\n");
    ...
    println!("\n=== RUN114 Aroon Regime Confirmation Results ===");
    ...
    let notes = format!("RUN114 Aroon confirmation. {} configs. Baseline PnL={:.2}. Best: {} (PnL={:.2}, Δ={:+.2}, filter_rate={:.1}%)",
    ...
    std::fs::write("/home/scamarena/ProjectCoin/run114_1_results.json", &serde_json::to_string_pretty(&output).unwrap()).ok();
    eprintln!("\nSaved → run114_1_results.json");
}
