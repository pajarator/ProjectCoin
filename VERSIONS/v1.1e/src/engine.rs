...
pub fn check_exit(state: &mut SharedState, ci: usize) -> bool {
    ...
    if pos.dir == "long" {
        ...
        if pnl > 0.0 && held >= config::MIN_HOLD_CANDLES {
            ...
            // RUN88: trailing z-score exit — exit when z has recovered Z_RECOVERY_PCT toward 0
    ...
    match mode {
        MarketMode::Long => {
            ...
            if strategies::long_entry(&ind, cfg.long_strat) {
                open_position(state, ci, ind.p, &regime.to_string(),
                    &cfg.long_strat.to_string(), Direction::Long, TradeType::Regime, Some(&ind));
            } else {
                // Try ISO short
                if strategies::iso_short_entry(&ind, cfg.iso_short_strat, ctx) {
                    state.coins[ci].active_strat = Some(cfg.iso_short_strat.to_string());
                    open_position(state, ci, ind.p, &regime.to_string(),
                        &cfg.iso_short_strat.to_string(), Direction::Short, TradeType::Regime, Some(&ind));
                }
            }
        }
        MarketMode::IsoShort => {
        ...
        MarketMode::Short => {
            ...
            if strategies::short_entry(&ind, cfg.short_strat) {
                open_position(state, ci, ind.p, &regime.to_string(),
                    &cfg.short_strat.to_string(), Direction::Short, TradeType::Regime, Some(&ind));
            }
        }
    }
}
...
pub fn check_scalp_entry(state: &mut SharedState, ci: usize) {
    ...
    if let Some((dir, strat_name)) = strategies::scalp_entry_with_price(&ind_1m, price) {
        ...
        open_position(state, ci, price, &regime.to_string(), strat_name, dir, TradeType::Scalp, None);
    }
}
...
fn open_position(
    ...
    entry_ind: Option<&crate::indicators::Ind15m>,
) {
    let cs = &mut state.coins[ci];
    if cs.pos.is_some() { return; }
    ...
    cs.pos = Some(Position {
        ...
        entry_z: if trade_type == TradeType::Regime { entry_ind.map(|i| i.z) } else { None },
    });
    cs.candles_held = 0;
