/// RUN46 — Partial Reversion Exit: Z-Score Deviation-Adaptive Exit
///
/// Exit when a fixed % of the z-score reversion has been captured.
/// E.g., enter at z=-2.5, exit when z=-0.875 (65% reversion).
///
/// Grid: REVERSION_CAPTURE_PCT: [0.50, 0.60, 0.70, 0.80, 0.90] + baseline
///
/// Run: cargo run --release --features run46 -- --run46
...
struct RevCfg { reversion_pct: f64 }

impl RevCfg {
    fn label(&self) -> String {
        if self.reversion_pct >= 999.0 { "DISABLED".to_string() }
        else { format!("R{:.0}", (self.reversion_pct * 100.0) as usize) }
    }
}

fn build_grid() -> Vec<RevCfg> {
    vec![
        RevCfg { reversion_pct: 999.0 }, // disabled
        RevCfg { reversion_pct: 0.50 },
        RevCfg { reversion_pct: 0.60 },
        RevCfg { reversion_pct: 0.70 },
        RevCfg { reversion_pct: 0.80 },
        RevCfg { reversion_pct: 0.90 },
    ]
}
...
#[derive(Serialize)]
struct CoinResult { coin: String, pnl: f64, trades: usize, wins: usize, losses: usize, wr: f64, pf: f64 }
#[derive(Serialize)]
struct ConfigResult { label: String, total_pnl: f64, portfolio_wr: f64, total_trades: usize, pf: f64, is_baseline: bool, coins: Vec<CoinResult> }
#[derive(Serialize)]
...
fn simulate(d: &CoinData15m, cfg: RevCfg) -> (f64, usize, usize, usize) {
    let n = d.closes.len();
    let mut bal = INITIAL_BAL;
    let mut pos: Option<(i8, f64, f64)> = None; // dir, entry, z_at_entry
    let mut cooldown = 0usize;
    let mut wins = 0usize; let mut losses = 0usize; let mut flats = 0usize;
    let disabled = cfg.reversion_pct >= 999.0;

    for i in 1..n {
        if let Some((dir, entry, z_entry)) = pos {
            ...
            if !closed {
                ...
                if !z_curr.is_nan() {
                    ...
                    if !disabled && z_entry.abs() > 0.1 {
                        let reversion = if z_entry < 0.0 {
                            // Long: z went from neg to less neg
                            (z_entry - z_curr) / z_entry
                        } else {
                            // Short: z went from pos to less pos
                            (z_entry - z_curr) / z_entry
                        };
                        if reversion >= cfg.reversion_pct && i > 0 { exit_pct = pct; closed = true; }
                    }
                }
            }
            if !closed {
                // Standard z-score crossback exit
            ...
            if let Some(dir) = regime_signal(d, i) {
                if i+1 < n {
                    ...
                    if entry_price > 0.0 && !z_entry.is_nan() {
                        pos = Some((dir, entry_price, z_entry));
                    }
                }
            }
        }
...
pub fn run(shutdown: Arc<AtomicBool>) {
    eprintln!("RUN46 — Partial Reversion Exit\n");
    ...
    let results: Vec<ConfigResult> = grid.par_iter().map(|cfg| {
        if shutdown.load(Ordering::SeqCst) {
            return ConfigResult { label: cfg.label(), total_pnl: 0.0, portfolio_wr: 0.0, total_trades: 0, pf: 0.0, is_baseline: cfg.reversion_pct >= 999.0, coins: vec![] };
        }
        let coin_results: Vec<CoinResult> = coin_data.iter().map(|d| {
            ...
            CoinResult { coin: d.name.clone(), pnl, trades, wins, losses, wr, pf: 0.0 }
        }).collect();
        ...
        let is_baseline = cfg.reversion_pct >= 999.0;
    ...
    println!("\n=== RUN46 Partial Reversion Exit Results ===");
    ...
    let notes = format!("RUN46 partial reversion exit. {} configs. Baseline PnL={:.2}. Best: {} (PnL={:.2}, Δ={:+.2})",
    ...
    std::fs::write("/home/scamarena/ProjectCoin/run46_1_results.json", &serde_json::to_string_pretty(&output).unwrap()).ok();
    eprintln!("\nSaved → run46_1_results.json");
}
