/// RUN105 — Z-Score Persistence Filter: Require Sustained Extreme Z-Score Before Entry
///
/// Grid: Z_THRESH [1.3, 1.5, 1.8] × Z_BARS [2, 3, 4] + baseline
/// 10 configs × 18 coins = 180 simulations (parallel per coin)
///
/// Run: cargo run --release --features run105 -- --run105
...
impl ZPersistCfg {
    fn label(&self) -> String {
        ...
        else { format!("ZT{:.1}_ZB{}", self.z_thresh, self.z_bars) }
    }
}

fn build_grid() -> Vec<ZPersistCfg> {
    let mut grid = vec![ZPersistCfg { z_thresh: 2.0, z_bars: 1, is_baseline: true }];
    for &zt in &[1.3, 1.5, 1.8] {
        for &zb in &[2usize, 3, 4] {
            grid.push(ZPersistCfg { z_thresh: zt, z_bars: zb, is_baseline: false });
        }
    }
    grid
}
...
fn simulate(d: &CoinData, cfg: &ZPersistCfg) -> CoinResult {
    let n = d.closes.len();
    ...
    // Persistence counter: consecutive bars z has been at/beyond threshold
    let mut z_persist_count: i32 = 0;
    let mut last_dir: Option<i8> = None;
    ...
    for i in 20..n {
        ...
        if let Some((dir, entry, entry_bar, _z_entry)) = pos.as_mut() {
            ...
            // Update persistence counter before checking entry
            let z = d.zscore[i];
            let is_long_extreme = z < -cfg.z_thresh;
            let is_short_extreme = z > cfg.z_thresh;

            if cfg.is_baseline {
                // Baseline: no persistence filter
                if let Some(dir) = regime_signal(z) {
                    if i + 1 < n {
                ...
                // Persistence mode: track consecutive bars at extreme
                if is_long_extreme {
                    if last_dir == Some(1) {
                        z_persist_count += 1;
                    } else {
                ...
                // Enter if persistence condition is met
                let persist_met = z_persist_count as usize >= cfg.z_bars;
                if persist_met && pos.is_none() {
                    if last_dir == Some(1) && is_long_extreme {
                        if i + 1 < n {
                            let entry_price = d.opens[i + 1];
                            if entry_price > 0.0 {
                                pos = Some((1, entry_price, i, z));
                                z_persist_count = 0;
                                last_dir = None;
                            }
                        }
                    } else if last_dir == Some(-1) && is_short_extreme {
                        if i + 1 < n {
                            let entry_price = d.opens[i + 1];
                            if entry_price > 0.0 {
                                pos = Some((-1, entry_price, i, z));
                ...
                // Track blocked entries (z reached regime threshold but persistence not met)
                if let Some(_dir) = regime_signal(z) {
                    if !persist_met {
                        entries_blocked += 1;
                    }
                }
    ...
    CoinResult { coin: d.name.clone(), pnl, trades: total_trades, wins, losses, flats, wr, pf, entries_blocked }
}

pub fn run(shutdown: Arc<AtomicBool>) {
    eprintln!("RUN105 — Z-Score Persistence Filter Grid Search\n");
    ...
    let all_results: Vec<ConfigResult> = grid.par_iter().map(|cfg| {
        if shutdown.load(Ordering::SeqCst) {
            return ConfigResult { label: cfg.label(), total_pnl: 0.0, portfolio_wr: 0.0, total_trades: 0, wins: 0, losses: 0, pf: 0.0, is_baseline: cfg.is_baseline, coins: vec![], entries_blocked_total: 0, block_rate: 0.0 };
        }
        ...
        let entries_blocked_total: usize = coin_results.iter().map(|c| c.entries_blocked).sum();
        let total_regime_signals = total_trades + entries_blocked_total;
        let block_rate = if total_regime_signals > 0 { entries_blocked_total as f64 / total_regime_signals as f64 * 100.0 } else { 0.0 };

        let d = done.fetch_add(N_COINS, Ordering::SeqCst) + N_COINS;
        eprintln!("  [{:>4}/{}] {}  PnL={:>+8.2}  WR={:>5.1}%  trades={}  blocked={} ({:.1}%)",
            d, total_sims, cfg.label(), total_pnl, portfolio_wr, total_trades, entries_blocked_total, block_rate);

        ConfigResult { label: cfg.label(), total_pnl, portfolio_wr, total_trades, wins: total_wins, losses: total_losses, pf, is_baseline: cfg.is_baseline, coins: coin_results, entries_blocked_total, block_rate }
    }).collect();
    ...
    println!("\n=== RUN105 Z-Score Persistence Filter Results ===");
    ...
    for (i, r) in sorted.iter().enumerate().take(15) {
        ...
        println!("{:>3}  {:<18} {:>+8.2} {:>+8.2} {:>5.1}%  {:>6} {:>8} {:>7.1}%",
            i+1, r.label, r.total_pnl, delta, r.portfolio_wr, r.total_trades, r.entries_blocked_total, r.block_rate);
    }
    println!("{}", "=".repeat(72));
    ...
    let notes = format!("RUN105 z-score persistence filter. {} configs. Baseline PnL={:.2}. Best: {} (PnL={:.2}, Δ={:+.2}, block_rate={:.1}%)",
        all_results.len(), baseline.total_pnl, best.label, best.total_pnl, best.total_pnl - baseline.total_pnl, best.block_rate);
    let output = Output { notes, configs: all_results };
    std::fs::write("/home/scamarena/ProjectCoin/run105_1_results.json", &serde_json::to_string_pretty(&output).unwrap()).ok();
    eprintln!("\nSaved → run105_1_results.json");
}
