/// RUN119 — Vortex Indicator Confirmation: VI+/- Directional Filter for Regime Entries
///
/// Grid: VI_LOOKBACK [14, 20, 30] × CROSS_STRICT [true, false]
/// 6 configs × 18 coins = 108 simulations (parallel per coin)
///
/// Run: cargo run --release --features run119 -- --run119
...
impl VortexCfg {
    fn label(&self) -> String {
        if self.is_baseline { "BASELINE".to_string() }
            ...
            let cs = if self.cross_strict { "CS" } else { "NS" };
            format!("LB{}_{}", self.lookback, cs)
        }
    }
}

fn build_grid() -> Vec<VortexCfg> {
    let mut grid = vec![VortexCfg { lookback: 0, cross_strict: false, is_baseline: true }];
    for &lb in &[14usize, 20, 30] {
        for &cs in &[true, false] {
            grid.push(VortexCfg { lookback: lb, cross_strict: cs, is_baseline: false });
        }
    }
    grid
}
...
fn compute_vortex(highs: &[f64], lows: &[f64], closes: &[f64], atr: &[f64], period: usize) -> (Vec<f64>, Vec<f64>) {
    let n = closes.len();
    let mut vi_plus = vec![f64::NAN; n];
    let mut vi_minus = vec![f64::NAN; n];
    let alpha = 2.0 / (period as f64 + 1.0);

    let mut ema_plus = 0.0;
    let mut ema_minus = 0.0;
    let mut initialized = false;

    for i in 1..n {
        if atr[i].is_nan() || atr[i] < 1e-10 { continue; }
        ...
        let vm_plus = (hp - lp).max(cp - cpp).max(hp - cpp);
        let vm_minus = (lp - hp).max(cp - cpp).max(lp - cpp);

        let ratio_plus = vm_plus / atr[i];
        let ratio_minus = vm_minus / atr[i];

        if !initialized {
            if i >= period {
                ema_plus = ratio_plus;
                ema_minus = ratio_minus;
                initialized = true;
                vi_plus[i] = ema_plus;
                vi_minus[i] = ema_minus;
            }
        } else {
            ema_plus = alpha * ratio_plus + (1.0 - alpha) * ema_plus;
            ema_minus = alpha * ratio_minus + (1.0 - alpha) * ema_minus;
            vi_plus[i] = ema_plus;
            vi_minus[i] = ema_minus;
        }
    }
...
fn load_15m(coin: &str) -> Option<CoinData> {
    ...
    let (vi_plus, vi_minus) = compute_vortex(&highs, &lows, &closes, &atr, 14);

    Some(CoinData { name: coin.to_string(), opens, closes, highs, lows, zscore, vi_plus, vi_minus })
}
...
fn simulate(d: &CoinData, cfg: &VortexCfg) -> CoinResult {
    let n = d.closes.len();
        ...
        if let Some((dir, entry, entry_bar)) = pos.as_mut() {
            ...
            if let Some(dir) = regime_signal(d.zscore[i]) {
                if cfg.is_baseline {
                    ...
                    let vp = d.vi_plus[i];
                    let vm = d.vi_minus[i];
                    let passes_filter = if !vp.is_nan() && !vm.is_nan() {
                        if cfg.cross_strict {
                            if dir == 1 { vm > vp * 1.1 }
                            else { vp > vm * 1.1 }
                        } else {
...
pub fn run(shutdown: Arc<AtomicBool>) {
    eprintln!("RUN119 — Vortex Indicator Confirmation Grid Search\n");
    ...
    println!("\n=== RUN119 Vortex Indicator Confirmation Results ===");
    ...
    let notes = format!("RUN119 Vortex indicator. {} configs. Baseline PnL={:.2}. Best: {} (PnL={:.2}, Δ={:+.2})",
    ...
    std::fs::write("/home/scamarena/ProjectCoin/run119_1_results.json", &serde_json::to_string_pretty(&output).unwrap()).ok();
    eprintln!("\nSaved → run119_1_results.json");
}
