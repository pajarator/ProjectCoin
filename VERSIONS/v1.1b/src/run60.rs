...
///
/// Grid: Z_MOMENTUM_THRESHOLD [0.02, 0.05, 0.10, 0.15] × Z_MOMENTUM_LOOKBACK [1, 2]
/// Total: 8 configs + baseline = 9
///
/// Run: cargo run --release --features run60 -- --run60
...
impl ZMomCfg {
    fn label(&self) -> String {
        if self.threshold == 0.0 { "DISABLED".to_string() }
        else { format!("ZM{:.2}_LB{}", self.threshold, self.lookback) }
    }
}

fn build_grid() -> Vec<ZMomCfg> {
    let mut grid = vec![ZMomCfg { threshold: 0.0, lookback: 1 }]; // baseline
    let thresholds = [0.02, 0.05, 0.10, 0.15];
    let lookbacks = [1, 2];
    for &t in &thresholds {
        for &lb in &lookbacks {
            grid.push(ZMomCfg { threshold: t, lookback: lb });
        }
    }
    grid
}
...
fn passes_z_momentum(d: &CoinData15m, i: usize, dir: i8, cfg: ZMomCfg) -> bool {
    if cfg.threshold == 0.0 { return true; }
    if i < cfg.lookback { return true; }
    let z_delta = d.zscore[i] - d.zscore[i - cfg.lookback];
    if z_delta.is_nan() { return true; }
    match dir {
        1 => z_delta > cfg.threshold,  // LONG: z should be rising (recovering toward mean)
        -1 => z_delta < -cfg.threshold, // SHORT: z should be falling (moving away from mean)
        _ => true,
    }
}

fn simulate(d: &CoinData15m, cfg: ZMomCfg) -> (f64, usize, usize, usize) {
    let n = d.closes.len();
    ...
    for i in 1..n {
        if let Some((dir, entry)) = pos {
            ...
            if let Some(dir) = regime_signal(d, i) {
                // Z-momentum filter
                if !passes_z_momentum(d, i, dir, cfg) { continue; }
                if i+1 < n {
...
pub fn run(shutdown: Arc<AtomicBool>) {
    eprintln!("RUN60 — Z-Score Momentum Filter\n");
    ...
    let results: Vec<ConfigResult> = grid.par_iter().map(|cfg| {
        if shutdown.load(Ordering::SeqCst) {
            return ConfigResult { label: cfg.label(), total_pnl: 0.0, portfolio_wr: 0.0, total_trades: 0, pf: 0.0, is_baseline: cfg.threshold == 0.0 };
        }
        ...
        let is_baseline = cfg.threshold == 0.0;
    ...
    println!("\n=== RUN60 Z-Score Momentum Filter Results ===");
    ...
    let notes = format!("RUN60 z-score momentum filter. {} configs. Baseline PnL={:.2}. Best: {} (PnL={:.2}, Δ={:+.2})",
    ...
    std::fs::write("/home/scamarena/ProjectCoin/run60_1_results.json", &serde_json::to_string_pretty(&output).unwrap()).ok();
    eprintln!("\nSaved → run60_1_results.json");
}
