/// RUN133 — Ultimate Oscillator Confirmation: Multi-Timeframe Oscillator for Smoother Entry Confirmation
///
/// Grid: UO_LONG_MAX [25, 30, 35] × UO_SHORT_MIN [65, 70, 75]
/// 9 configs × 18 coins = 162 simulations (parallel per coin)
///
/// Run: cargo run --release --features run133 -- --run133
...
fn build_grid() -> Vec<UoCfg> {
    let mut grid = vec![UoCfg { long_max: 0.0, short_min: 100.0, is_baseline: true }];
    for &lm in &[25.0f64, 30.0, 35.0] {
        for &sm in &[65.0f64, 70.0, 75.0] {
            grid.push(UoCfg { long_max: lm, short_min: sm, is_baseline: false });
        }
    }
    grid
}
...
fn compute_ultimate_oscillator(closes: &[f64], highs: &[f64], lows: &[f64], p1: usize, p2: usize, p3: usize) -> Vec<f64> {
    let n = closes.len();
    let mut uo = vec![f64::NAN; n];
    if n < p3 + 1 { return uo; }

    // Precompute Buying Pressure (BP) and True Range (TR)
    let mut bp: Vec<f64> = vec![0.0; n];
    let mut tr: Vec<f64> = vec![0.0; n];
    for i in 1..n {
        let prev_close = closes[i - 1];
        bp[i] = closes[i] - lows[i].min(prev_close);
        tr[i] = highs[i].max(prev_close) - lows[i].min(prev_close);
    }

    // Compute averages for each period
    for i in p3..n {
        // Average for period 1 (p1)
...
fn load_15m(coin: &str) -> Option<CoinData> {
    ...
    let uo = compute_ultimate_oscillator(&closes, &highs, &lows, 7, 14, 28);

    Some(CoinData { name: coin.to_string(), opens, closes, highs, lows, zscore, uo })
}

fn simulate(d: &CoinData, cfg: &UoCfg) -> CoinResult {
    let n = d.closes.len();
        ...
        if let Some((dir, entry, entry_bar)) = pos.as_mut() {
            ...
            if let Some(dir) = regime_signal(d.zscore[i]) {
                if cfg.is_baseline {
                    ...
                    let uo_val = d.uo[i];
                    let passes_filter = if uo_val.is_nan() {
                        true
                    } else if dir == 1 {
                        // LONG: UO < long_max (deeply oversold)
                        uo_val < cfg.long_max
                    } else {
                        // SHORT: UO > short_min (deeply overbought)
                        uo_val > cfg.short_min
                    };
...
pub fn run(shutdown: Arc<AtomicBool>) {
    eprintln!("RUN133 — Ultimate Oscillator Confirmation Grid Search\n");
    ...
    println!("\n=== RUN133 Ultimate Oscillator Confirmation Results ===");
    ...
    let notes = format!("RUN133 Ultimate Oscillator filter. {} configs. Baseline PnL={:.2}. Best: {} (PnL={:.2}, Δ={:+.2})",
    ...
    std::fs::write("/home/scamarena/ProjectCoin/run133_1_results.json", &serde_json::to_string_pretty(&output).unwrap()).ok();
    eprintln!("\nSaved → run133_1_results.json");
}
