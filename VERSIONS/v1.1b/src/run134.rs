/// RUN134 — Connors RSI Confirmation: Short-Term RSI Streak for Faster Entry Confirmation
///
/// Grid: CONNORS_LONG_MAX [20, 25, 30] × CONNORS_SHORT_MIN [70, 75, 80]
/// 9 configs × 18 coins = 162 simulations (parallel per coin)
///
/// Run: cargo run --release --features run134 -- --run134
...
impl ConnorsCfg {
    fn label(&self) -> String {
        ...
        else { format!("LM{}_SM{}", self.long_max as usize, self.short_min as usize) }
    }
}

fn build_grid() -> Vec<ConnorsCfg> {
    let mut grid = vec![ConnorsCfg { long_max: 0.0, short_min: 100.0, is_baseline: true }];
    for &lm in &[20.0f64, 25.0, 30.0] {
        for &sm in &[70.0f64, 75.0, 80.0] {
            grid.push(ConnorsCfg { long_max: lm, short_min: sm, is_baseline: false });
        }
    }
    grid
}
...
fn percentile_rank(data: &[f64], current: f64) -> f64 {
    if data.is_empty() { return 50.0; }
    let more_extreme = data.iter().filter(|&&x| x <= current).count();
    (more_extreme as f64 / data.len() as f64) * 100.0
}
...
fn load_15m(coin: &str) -> Option<CoinData> {
    ...
    // Component 1: RSI(price, 3)
    let rsi_price = compute_rsi(&closes, 3);

    // Component 2: RSI(streak, 2)
    // Compute streak series: +1 per up close, -1 per down close, 0 on flat
    let mut streak = vec![0i32; n];
    for i in 1..n {
        if closes[i] > closes[i-1] { streak[i] = streak[i-1] + 1; }
        else if closes[i] < closes[i-1] { streak[i] = streak[i-1] - 1; }
        else { streak[i] = 0; }
    }
    // Convert streak to f64 for RSI computation
    let streak_f64: Vec<f64> = streak.iter().map(|&s| s as f64).collect();
    let rsi_streak = compute_rsi(&streak_f64, 2);

    // Component 3: PercentRank(price, 100)
    let pct_rank_period = 100;
    let mut connors_rsi = vec![f64::NAN; n];
    for i in pct_rank_period..n {
        let history = &closes[i+1-pct_rank_period..i];
        let pct_rank = percentile_rank(history, closes[i]);
        let rsi_p = rsi_price[i];
        let rsi_s = rsi_streak[i];
        if !rsi_p.is_nan() && !rsi_s.is_nan() {
            connors_rsi[i] = (rsi_p + rsi_s + pct_rank) / 3.0;
        }
    }

    Some(CoinData { name: coin.to_string(), opens, closes, zscore, connors_rsi })
}

fn simulate(d: &CoinData, cfg: &ConnorsCfg) -> CoinResult {
    let n = d.closes.len();
        ...
        if let Some((dir, entry, entry_bar)) = pos.as_mut() {
            ...
            if let Some(dir) = regime_signal(d.zscore[i]) {
                if cfg.is_baseline {
                    ...
                    let crsi = d.connors_rsi[i];
                    let passes_filter = if crsi.is_nan() {
                        true
                    } else if dir == 1 {
                        crsi < cfg.long_max
                    } else {
...
pub fn run(shutdown: Arc<AtomicBool>) {
    eprintln!("RUN134 — Connors RSI Confirmation Grid Search\n");
    ...
    println!("\n=== RUN134 Connors RSI Confirmation Results ===");
    ...
    let notes = format!("RUN134 Connors RSI filter. {} configs. Baseline PnL={:.2}. Best: {} (PnL={:.2}, Δ={:+.2})",
    ...
    std::fs::write("/home/scamarena/ProjectCoin/run134_1_results.json", &serde_json::to_string_pretty(&output).unwrap()).ok();
    eprintln!("\nSaved → run134_1_results.json");
}
