...
///
/// Grid: Z_WINDOW [50, 100, 200] × PERC_LONG [5, 10, 15] × PERC_SHORT [85, 90, 95]
/// 27 configs × 18 coins = 486 simulations (parallel per coin)
///
/// Run: cargo run --release --features run107 -- --run107
...
impl PercZCfg {
    fn label(&self) -> String {
        ...
        else { format!("ZW{}_PL{:.0}_PS{:.0}", self.z_window, self.perc_long, self.perc_short) }
    }
}

fn build_grid() -> Vec<PercZCfg> {
    let mut grid = vec![PercZCfg { z_window: 100, perc_long: 100.0, perc_short: 0.0, is_baseline: true }];
    for &zw in &[50usize, 100, 200] {
        for &pl in &[5.0, 10.0, 15.0] {
            for &ps in &[85.0, 90.0, 95.0] {
                grid.push(PercZCfg { z_window: zw, perc_long: pl, perc_short: ps, is_baseline: false });
            }
        }
    }
    grid
...
/// Compute percentile rank of current z in rolling window.
/// Returns % of historical z-scores more extreme than current.
fn z_percentile_rank(z_history: &[f64], current_z: f64) -> f64 {
    if z_history.is_empty() { return 50.0; }
    let n = z_history.len();
    // Count how many historical z-scores are MORE extreme than current
    let more_extreme = z_history.iter()
        .filter(|&&z| {
            if current_z < 0.0 { z < current_z } // more negative = more extreme for longs
            else { z > current_z }                  // more positive = more extreme for shorts
        })
        .count();
    (more_extreme as f64 / n as f64) * 100.0
}
...
fn simulate(d: &CoinData, cfg: &PercZCfg) -> CoinResult {
    let n = d.closes.len();
    ...
    // Rolling z-score history for percentile computation
    let mut z_history: Vec<f64> = Vec::with_capacity(cfg.z_window.max(1));

    for i in 20..n {
        cooldown = cooldown.saturating_sub(1);

        // Update z-history (at least z_window bars accumulated before filtering)
        if !d.zscore[i-1].is_nan() {
            z_history.push(d.zscore[i-1]);
            if z_history.len() > cfg.z_window {
                z_history.remove(0);
            }
        }
        ...
        if let Some((dir, entry, entry_bar, _z_entry)) = pos.as_mut() {
            ...
            if let Some(dir) = regime_signal(d.zscore[i]) {
                if cfg.is_baseline {
                    ...
                    // Percentile filter: only enter if z is in the most extreme X%
                    let pct_rank = z_percentile_rank(&z_history, d.zscore[i]);
                    let passes_filter = if dir == 1 {
                        pct_rank <= cfg.perc_long
                    } else {
                        pct_rank >= cfg.perc_short
                    };
                    if passes_filter {
                        if i + 1 < n {
                            let entry_price = d.opens[i + 1];
...
pub fn run(shutdown: Arc<AtomicBool>) {
    eprintln!("RUN107 — Percentile Rank Z-Filter Grid Search\n");
    ...
    println!("\n=== RUN107 Percentile Rank Z-Filter Results ===");
    ...
    let notes = format!("RUN107 percentile rank z-filter. {} configs. Baseline PnL={:.2}. Best: {} (PnL={:.2}, Δ={:+.2}, filter_rate={:.1}%)",
    ...
    std::fs::write("/home/scamarena/ProjectCoin/run107_1_results.json", &serde_json::to_string_pretty(&output).unwrap()).ok();
    eprintln!("\nSaved → run107_1_results.json");
}
