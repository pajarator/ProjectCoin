...
///
/// Grid: RSI_DIV_LOOKBACK [10, 14, 20] × RSI_DIV_TOLERANCE [0.01, 0.02, 0.05]
/// 9 configs × 18 coins = 162 simulations (parallel per coin)
///
/// Run: cargo run --release --features run132 -- --run132
...
impl RsiDivCfg {
    fn label(&self) -> String {
        ...
        else { format!("LB{}_T{}", self.lookback, (self.tolerance * 100.0) as usize) }
    }
}

fn build_grid() -> Vec<RsiDivCfg> {
    let mut grid = vec![RsiDivCfg { lookback: 0, tolerance: 0.0, is_baseline: true }];
    for &lb in &[10usize, 14, 20] {
        for &tol in &[0.01f64, 0.02, 0.05] {
            grid.push(RsiDivCfg { lookback: lb, tolerance: tol, is_baseline: false });
        }
    }
    grid
}
...
fn compute_rsi(closes: &[f64], period: usize) -> Vec<f64> {
    ...
    for i in period..n {
        ...
        let avg_gain = (avg_gain * (period - 1) as f64 + gain) / period as f64;
        let avg_loss = (avg_loss * (period - 1) as f64 + loss) / period as f64;
...
fn load_15m(coin: &str) -> Option<CoinData> {
    ...
    Some(CoinData { name: coin.to_string(), opens, closes, highs, lows, zscore, rsi })
}

fn has_bullish_divergence(d: &CoinData, i: usize, cfg: &RsiDivCfg) -> bool {
    if i < cfg.lookback { return false; }
    let rsi_curr = d.rsi[i];
    if rsi_curr.is_nan() { return false; }

    // Price: current low vs low at lookback
    let curr_low = d.lows[i];
    let prior_low = d.lows[i - cfg.lookback];
    // RSI: current RSI vs RSI at lookback
    let rsi_prior = d.rsi[i - cfg.lookback];
    if rsi_prior.is_nan() { return false; }

    // Bullish hidden divergence: price makes new low (lower than prior low)
    // RSI makes higher low (not confirmed - divergence)
    let tol = cfg.tolerance;
    let price_lower = curr_low < prior_low * (1.0 - tol);
    let rsi_higher = rsi_curr > rsi_prior * (1.0 - tol);

    price_lower && rsi_higher
}

fn has_bearish_divergence(d: &CoinData, i: usize, cfg: &RsiDivCfg) -> bool {
    if i < cfg.lookback { return false; }
    let rsi_curr = d.rsi[i];
    if rsi_curr.is_nan() { return false; }

    // Price: current high vs high at lookback
    let curr_high = d.highs[i];
    let prior_high = d.highs[i - cfg.lookback];
    // RSI: current RSI vs RSI at lookback
    let rsi_prior = d.rsi[i - cfg.lookback];
    if rsi_prior.is_nan() { return false; }

    // Bearish hidden divergence: price makes new high (higher than prior high)
    // RSI makes lower high (not confirmed - divergence)
    let tol = cfg.tolerance;
    let price_higher = curr_high > prior_high * (1.0 + tol);
    let rsi_lower = rsi_curr < rsi_prior * (1.0 + tol);

    price_higher && rsi_lower
}

fn simulate(d: &CoinData, cfg: &RsiDivCfg) -> CoinResult {
    let n = d.closes.len();
            ...
            if let Some(dir) = regime_signal(d.zscore[i]) {
                if cfg.is_baseline {
                    ...
                    let passes_filter = if dir == 1 {
                        // LONG: require bullish RSI divergence
                        has_bullish_divergence(d, i, cfg)
                    } else {
                        // SHORT: require bearish RSI divergence
                        has_bearish_divergence(d, i, cfg)
                    };
...
pub fn run(shutdown: Arc<AtomicBool>) {
    eprintln!("RUN132 — RSI Divergence Confirmation Grid Search\n");
    ...
    println!("\n=== RUN132 RSI Divergence Confirmation Results ===");
    ...
    let notes = format!("RUN132 RSI divergence filter. {} configs. Baseline PnL={:.2}. Best: {} (PnL={:.2}, Δ={:+.2})",
    ...
    std::fs::write("/home/scamarena/ProjectCoin/run132_1_results.json", &serde_json::to_string_pretty(&output).unwrap()).ok();
    eprintln!("\nSaved → run132_1_results.json");
}
