...
///
/// Grid: TZI_WINDOW [10, 20, 30] × TZI_THRESH [50.0, 60.0, 70.0]
/// 9 configs × 18 coins = 162 simulations (parallel per coin)
///
/// Run: cargo run --release --features run136 -- --run136
...
impl TziCfg {
    fn label(&self) -> String {
        ...
        else { format!("W{}_T{}", self.window, self.thresh as usize) }
    }
}

fn build_grid() -> Vec<TziCfg> {
    let mut grid = vec![TziCfg { window: 0, thresh: 0.0, is_baseline: true }];
    for &w in &[10usize, 20, 30] {
        for &t in &[50.0f64, 60.0, 70.0] {
            grid.push(TziCfg { window: w, thresh: t, is_baseline: false });
        }
    }
    grid
}
...
fn compute_tzi_window(highs: &[f64], lows: &[f64], closes: &[f64], window: usize) -> Vec<f64> {
    let n = closes.len();
    let mut tzi = vec![50.0; n]; // Default 50 when not enough data
    for i in window..n {
        // Find min low and max high over window ending at i
        ...
        if range_size > 0.0 {
            let in_zone = (start..=i)
                .filter(|&j| closes[j] >= min_low && closes[j] <= max_high)
                .count();
            tzi[i] = (in_zone as f64) / (window as f64) * 100.0;
        }
    }
    tzi
}
...
fn simulate(d: &CoinData, cfg: &TziCfg) -> CoinResult {
    let n = d.closes.len();
    ...
    // Compute TZI for this config's window
    let tzi = compute_tzi_window(&d.highs, &d.lows, &d.closes, cfg.window);
        ...
        if let Some((dir, entry, entry_bar)) = pos.as_mut() {
            ...
            if let Some(dir) = regime_signal(d.zscore[i]) {
                if cfg.is_baseline {
                    ...
                    // Require TZI > thresh (market consolidating)
                    let tzi_val = tzi[i];
                    let passes_filter = tzi_val > cfg.thresh;
...
pub fn run(shutdown: Arc<AtomicBool>) {
    eprintln!("RUN136 — Trade Zone Index Confirmation Grid Search\n");
    ...
    println!("\n=== RUN136 Trade Zone Index Confirmation Results ===");
    ...
    let notes = format!("RUN136 TZI filter. {} configs. Baseline PnL={:.2}. Best: {} (PnL={:.2}, Δ={:+.2})",
    ...
    std::fs::write("/home/scamarena/ProjectCoin/run136_1_results.json", &serde_json::to_string_pretty(&output).unwrap()).ok();
    eprintln!("\nSaved → run136_1_results.json");
}
