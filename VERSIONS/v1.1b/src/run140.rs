...
///
/// Grid: KC_PERIOD [14, 20, 30] × KC_MULT [1.5, 2.0, 2.5]
/// 9 configs × 18 coins = 162 simulations (parallel per coin)
///
/// Run: cargo run --release --features run140 -- --run140
...
impl KcCfg {
    fn label(&self) -> String {
        ...
        else { format!("P{}_M{}", self.period, self.mult) }
    }
}

fn build_grid() -> Vec<KcCfg> {
    let mut grid = vec![KcCfg { period: 0, mult: 0.0, is_baseline: true }];
    for &p in &[14usize, 20, 30] {
        for &m in &[1.5f64, 2.0, 2.5] {
            grid.push(KcCfg { period: p, mult: m, is_baseline: false });
        }
    }
    grid
}
...
struct CoinData {
    ...
    lows: Vec<f64>,
    zscore: Vec<f64>,
    keltner_upper: Vec<f64>,
    keltner_lower: Vec<f64>,
}
...
fn load_15m(coin: &str) -> Option<CoinData> {
    ...
    Some(CoinData { name: coin.to_string(), opens, closes, highs, lows, zscore, keltner_upper: vec![], keltner_lower: vec![] })
}

fn simulate(d: &CoinData, cfg: &KcCfg) -> CoinResult {
    let n = d.closes.len();
    ...
    // Compute Keltner Channel for this config
    let ema = compute_ema(&d.closes, cfg.period);
    let atr = compute_atr(&d.highs, &d.lows, &d.closes, cfg.period);
    let mut keltner_upper = vec![f64::NAN; n];
    let mut keltner_lower = vec![f64::NAN; n];
    for i in cfg.period..n {
        if !ema[i].is_nan() && !atr[i].is_nan() {
            keltner_upper[i] = ema[i] + cfg.mult * atr[i];
            keltner_lower[i] = ema[i] - cfg.mult * atr[i];
        }
    }
        ...
        if let Some((dir, entry, entry_bar)) = pos.as_mut() {
            ...
            if let Some(dir) = regime_signal(d.zscore[i]) {
                if cfg.is_baseline {
                    ...
                    let ku = keltner_upper[i];
                    let kl = keltner_lower[i];
                    let passes_filter = if ku.is_nan() || kl.is_nan() {
                        true
                    } else if dir == 1 {
                        // LONG: price must be within channel (not broken out above)
                        d.closes[i] < ku
                    } else {
                        // SHORT: price must be within channel (not broken out below)
                        d.closes[i] > kl
                    };
...
pub fn run(shutdown: Arc<AtomicBool>) {
    eprintln!("RUN140 — Keltner Channel Breakout Filter Grid Search\n");
    ...
    println!("\n=== RUN140 Keltner Channel Breakout Filter Results ===");
    ...
    let notes = format!("RUN140 Keltner Channel filter. {} configs. Baseline PnL={:.2}. Best: {} (PnL={:.2}, Δ={:+.2})",
    ...
    std::fs::write("/home/scamarena/ProjectCoin/run140_1_results.json", &serde_json::to_string_pretty(&output).unwrap()).ok();
    eprintln!("\nSaved → run140_1_results.json");
}
