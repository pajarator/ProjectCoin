...
///
/// Grid: SUPPRESS_MODE [1=suppress_bad, 2=allow_good] × MON_ALLOWED × FRI_ALLOWED × WEEKEND_ALLOWED
/// Total: 16 configs + baseline = 17
///
/// Run: cargo run --release --features run57 -- --run57
...
struct DowCfg {
    suppress_mode: u8,  // 1=suppress_bad_days, 2=allow_good_days_only
    monday_allowed: bool,
    friday_allowed: bool,
...
impl DowCfg {
    fn label(&self) -> String {
        if self.suppress_mode == 0 && !self.monday_allowed && !self.friday_allowed && !self.weekend_allowed {
            "DISABLED".to_string()
        } else {
            format!("SM{}_M{}_F{}_W{}",
                self.suppress_mode,
                if self.monday_allowed { 1 } else { 0 },
                if self.friday_allowed { 1 } else { 0 },
                if self.weekend_allowed { 1 } else { 0 })
        }
    }
}

fn build_grid() -> Vec<DowCfg> {
    let mut grid = vec![DowCfg { suppress_mode: 0, monday_allowed: true, friday_allowed: true, weekend_allowed: true }]; // baseline
    let suppress_modes = [1, 2];
    let monday = [true, false];
    let friday = [true, false];
    let weekend = [true, false];
    for &sm in &suppress_modes {
        for &m in &monday {
            for &f in &friday {
                for &w in &weekend {
                    grid.push(DowCfg { suppress_mode: sm, monday_allowed: m, friday_allowed: f, weekend_allowed: w });
                }
            }
        }
    }
...
struct CoinData15m { name: String, closes: Vec<f64>, opens: Vec<f64>, zscore: Vec<f64>, dow: Vec<u8> }
...
fn load_15m(coin: &str) -> Option<CoinData15m> {
    ...
    for line in data.lines().skip(1) {
        ...
        // Parse date string "YYYY-MM-DD HH:MM:SS" to extract day-of-week
        let day = chrono::NaiveDate::parse_from_str(&ts_str[..10], "%Y-%m-%d")
            .ok()?.weekday().num_days_from_monday() as u8;
        opens.push(oo); closes.push(cc); dow.push(day);
    }
    ...
    Some(CoinData15m { name: coin.to_string(), closes, opens, zscore, dow })
}
...
fn is_good_day(dow: u8, cfg: DowCfg) -> bool {
    let is_monday = dow == 0;
    let is_friday = dow == 4;
    let is_weekend = dow == 5 || dow == 6;

    let monday_ok = cfg.monday_allowed || !is_monday;
    let friday_ok = cfg.friday_allowed || !is_friday;
    let weekend_ok = cfg.weekend_allowed || (!is_weekend);

    monday_ok && friday_ok && weekend_ok
}

fn passes_dow_filter(dow: u8, cfg: DowCfg) -> bool {
    if cfg.suppress_mode == 0 { return true; } // disabled
    match cfg.suppress_mode {
        1 => is_good_day(dow, cfg), // suppress bad days
        2 => !is_good_day(dow, cfg), // allow only good days (block bad)
        _ => true,
    }
}

fn simulate(d: &CoinData15m, cfg: DowCfg) -> (f64, usize, usize, usize) {
    let n = d.closes.len();
    ...
    for i in 1..n {
        if let Some((dir, entry)) = pos {
            ...
            if let Some(dir) = regime_signal(d, i) {
                // Day-of-week filter
                if !passes_dow_filter(d.dow[i], cfg) { continue; }
                if i+1 < n {
...
pub fn run(shutdown: Arc<AtomicBool>) {
    eprintln!("RUN57 — Day-of-Week Trade Filter\n");
    ...
    let results: Vec<ConfigResult> = grid.par_iter().map(|cfg| {
        if shutdown.load(Ordering::SeqCst) {
            return ConfigResult { label: cfg.label(), total_pnl: 0.0, portfolio_wr: 0.0, total_trades: 0, pf: 0.0, is_baseline: cfg.suppress_mode == 0 };
        }
        ...
        let is_baseline = cfg.suppress_mode == 0;
    ...
    println!("\n=== RUN57 Day-of-Week Filter Results ===");
    println!("Baseline: PnL={:+.2}  WR={:.1}%  Trades={}", baseline.total_pnl, baseline.portfolio_wr, baseline.total_trades);
    println!("\n{:>3}  {:<14} {:>8} {:>8} {:>8} {:>8}", "#", "Config", "PnL", "ΔPnL", "WR%", "Trades");
    println!("{}", "-".repeat(58));
    for (i,r) in sorted.iter().enumerate() {
        let delta = r.total_pnl - baseline.total_pnl;
        println!("{:>3}  {:<14} {:>+8.2} {:>+8.2} {:>6.1}% {:>6}", i+1, r.label, r.total_pnl, delta, r.portfolio_wr, r.total_trades);
    }
    println!("{}", "=".repeat(58));
    ...
    let notes = format!("RUN57 day-of-week filter. {} configs. Baseline PnL={:.2}. Best: {} (PnL={:.2}, Δ={:+.2})",
    ...
    std::fs::write("/home/scamarena/ProjectCoin/run57_1_results.json", &serde_json::to_string_pretty(&output).unwrap()).ok();
    eprintln!("\nSaved → run57_1_results.json");
}
