/// RUN118 — Hull Suite Trend Filter: HMA Low-Lag Trend Confirmation for Entries and Exits
///
/// Grid: HULL_LOOKBACK [10, 20, 30] × SLOPE_CONFIRM [true, false] × HULL_EXIT [true, false]
/// 12 configs × 18 coins = 216 simulations (parallel per coin)
///
/// Run: cargo run --release --features run118 -- --run118
...
impl HullCfg {
    fn label(&self) -> String {
        if self.is_baseline { "BASELINE".to_string() }
            ...
            let sc = if self.slope_confirm { "SC" } else { "NS" };
            let he = if self.hull_exit { "HE" } else { "NH" };
            format!("LB{}_{}_{}", self.lookback, sc, he)
        }
    }
}

fn build_grid() -> Vec<HullCfg> {
    let mut grid = vec![HullCfg { lookback: 0, slope_confirm: false, hull_exit: false, is_baseline: true }];
    for &lb in &[10usize, 20, 30] {
        for &sc in &[true, false] {
            for &he in &[true, false] {
                grid.push(HullCfg { lookback: lb, slope_confirm: sc, hull_exit: he, is_baseline: false });
            }
        }
    }
    grid
...
struct CoinResult {
    ...
    hull_exits: usize,
    hull_exit_wins: usize,
    hull_exit_losses: usize,
}
#[derive(Serialize)]
struct ConfigResult {
    label: String,
...
fn load_15m(coin: &str) -> Option<CoinData> {
    ...
    // Default lookback=20
    let hma = compute_hma(&closes, 20);

    Some(CoinData { name: coin.to_string(), opens, closes, highs, lows, zscore, hma })
}
...
fn simulate(d: &CoinData, cfg: &HullCfg) -> CoinResult {
    let n = d.closes.len();
    ...
    let mut hull_exits = 0usize; let mut hull_exit_wins = 0usize; let mut hull_exit_losses = 0usize;

    // Precompute HMA for this lookback
    let hma = compute_hma(&d.closes, cfg.lookback);
    ...
    for i in cfg.lookback..n {
        ...
        if let Some((dir, entry, entry_bar)) = pos.as_mut() {
            ...
            // Hull exit
            if !closed && cfg.hull_exit && !cfg.is_baseline {
                let curr_hma = hma[i];
                let prev_hma = hma[i.saturating_sub(1)];
                if !curr_hma.is_nan() && !prev_hma.is_nan() {
                    let hull_exit = if *dir == 1 {
                        d.closes[i] > curr_hma && d.closes[i.saturating_sub(1)] <= prev_hma
                    } else {
                        d.closes[i] < curr_hma && d.closes[i.saturating_sub(1)] >= prev_hma
                    };
                    if hull_exit {
                        ...
                        if net > 1e-10 { hull_exit_wins += 1; }
                        else if net < -1e-10 { hull_exit_losses += 1; }
                    }
                }
            }
            ...
            if let Some(dir) = regime_signal(d.zscore[i]) {
                if cfg.is_baseline {
                    ...
                    let hma_val = hma[i];
                    let passes_filter = if !hma_val.is_nan() {
                        let price_ok = if dir == 1 { d.closes[i] < hma_val } else { d.closes[i] > hma_val };
                        let slope_ok = if cfg.slope_confirm {
                            let slope = hma[i] - hma[i.saturating_sub(2.min(i))];
                            if dir == 1 { slope < 0.0 } else { slope > 0.0 }
                        } else { true };
                        price_ok && slope_ok
                    } else { true };
    ...
    CoinResult { coin: d.name.clone(), pnl, trades: total_trades, wins, losses, flats, wr, pf, entries_filtered, hull_exits, hull_exit_wins, hull_exit_losses }
}

pub fn run(shutdown: Arc<AtomicBool>) {
    eprintln!("RUN118 — Hull Suite Trend Filter Grid Search\n");
    ...
    let all_results: Vec<ConfigResult> = grid.par_iter().map(|cfg| {
        if shutdown.load(Ordering::SeqCst) {
            return ConfigResult { label: cfg.label(), total_pnl: 0.0, portfolio_wr: 0.0, total_trades: 0, wins: 0, losses: 0, pf: 0.0, is_baseline: cfg.is_baseline, coins: vec![], entries_filtered_total: 0, filter_rate: 0.0, hull_exits_total: 0, hull_exit_wr: 0.0 };
        }
        ...
        let hull_exits_total: usize = coin_results.iter().map(|c| c.hull_exits).sum();
        let hull_exit_wr = if hull_exits_total > 0 { coin_results.iter().map(|c| c.hull_exit_wins).sum::<usize>() as f64 / hull_exits_total as f64 * 100.0 } else { 0.0 };

        let d = done.fetch_add(N_COINS, Ordering::SeqCst) + N_COINS;
        eprintln!("  [{:>4}/{}] {}  PnL={:>+8.2}  WR={:>5.1}%  trades={}  filtered={} hull_exits={}",
            d, total_sims, cfg.label(), total_pnl, portfolio_wr, total_trades, entries_filtered_total, hull_exits_total);

        ConfigResult { label: cfg.label(), total_pnl, portfolio_wr, total_trades, wins: total_wins, losses: total_losses, pf, is_baseline: cfg.is_baseline, coins: coin_results, entries_filtered_total, filter_rate, hull_exits_total, hull_exit_wr }
    }).collect();
    ...
    println!("\n=== RUN118 Hull Suite Trend Filter Results ===");
    println!("Baseline: PnL={:+.2}  WR={:.1}%  Trades={}", baseline.total_pnl, baseline.portfolio_wr, baseline.total_trades);
    println!("\n{:>3}  {:<22} {:>8} {:>8} {:>6} {:>7} {:>8} {:>7} {:>9}",
        "#", "Config", "PnL", "ΔPnL", "WR%", "Trades", "Filtered", "HullExits", "HE_WR%");
    println!("{}", "-".repeat(90));
    for (i, r) in sorted.iter().enumerate().take(15) {
        let delta = r.total_pnl - baseline.total_pnl;
        println!("{:>3}  {:<22} {:>+8.2} {:>+8.2} {:>5.1}%  {:>6} {:>8} {:>7} {:>8.1}%",
            i+1, r.label, r.total_pnl, delta, r.portfolio_wr, r.total_trades, r.entries_filtered_total, r.hull_exits_total, r.hull_exit_wr);
    }
    println!("{}", "=".repeat(90));
    ...
    let notes = format!("RUN118 Hull Suite. {} configs. Baseline PnL={:.2}. Best: {} (PnL={:.2}, Δ={:+.2})",
    ...
    std::fs::write("/home/scamarena/ProjectCoin/run118_1_results.json", &serde_json::to_string_pretty(&output).unwrap()).ok();
    eprintln!("\nSaved → run118_1_results.json");
}
