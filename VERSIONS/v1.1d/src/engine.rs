...
pub fn check_exit(state: &mut SharedState, ci: usize) -> bool {
    ...
    if pos.dir == "long" {
        ...
        // RUN82: regime decay exit — ADX has risen significantly above entry ADX
        if config::REGIME_DECAY_ENABLE && pnl > 0.0 && held >= config::REGIME_DECAY_GRACE_BARS {
...
fn open_position(
    ...
    // RUN82: record entry ADX for regime trades (for decay exit)
    let entry_adx = if trade_type == TradeType::Regime {
        cs.ind_15m.as_ref().map(|i| i.adx)
    } else {
        None
    };
