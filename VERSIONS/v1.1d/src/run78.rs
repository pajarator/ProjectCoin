/// RUN78 — Cross-Coin Z-Score Confirmation Filter
///
/// Require BTC z-score to confirm regime entries:
/// - LONG: require btc_z >= MIN_LONG
/// - SHORT: require btc_z <= -MAX_SHORT (symmetric)
/// - SUPPRESS zone: if |btc_z| < SUPPRESS, block all regime entries
///
/// Grid: MIN_LONG [0.5, 1.0, 1.5] × MAX_SHORT [0.5, 1.0, 1.5] × SUPPRESS [1.0, 1.5, 2.0]
/// Total: 3 × 3 × 3 = 27 + baseline = 28 configs
///
/// Run: cargo run --release --features run78 -- --run78
...
impl CrossZCfg {
    fn label(&self) -> String {
        ...
        else { format!("ML{:.1}_MS{:.1}_SUP{:.1}", self.min_long, self.max_short, self.suppress) }
    }
}

fn build_grid() -> Vec<CrossZCfg> {
    let mut grid = vec![CrossZCfg { min_long: 0.0, max_short: 0.0, suppress: 0.0, is_baseline: true }];
    let mins = [0.5, 1.0, 1.5];
    let maxs = [0.5, 1.0, 1.5];
    let sups = [1.0, 1.5, 2.0];
    for &ml in &mins {
        for &ms in &maxs {
            for &sup in &sups {
                grid.push(CrossZCfg { min_long: ml, max_short: ms, suppress: sup, is_baseline: false });
            }
        }
    }
    grid
...
// btc_z_confirmed returns (allowed: bool, blocked: bool)
fn btc_z_confirmed(btc_z: f64, dir: i8, cfg: &CrossZCfg) -> (bool, bool) {
    if cfg.is_baseline { return (true, false); }
    // Suppress zone: if |btc_z| < suppress, block all
    if btc_z.abs() < cfg.suppress { return (false, true); }
    // LONG: require btc_z >= min_long
    if dir == 1 && btc_z < cfg.min_long { return (false, true); }
    // SHORT: require btc_z <= -max_short
    if dir == -1 && btc_z > -cfg.max_short { return (false, true); }
    (true, false)
}

fn simulate(d: &CoinData15m, btc_zscore: &[f64], cfg: &CrossZCfg) -> (f64, usize, usize, usize, usize, usize) {
    let n = d.closes.len();
    ...
    for i in 1..n {
        let btc_z = btc_zscore.get(i).copied().unwrap_or(0.0);
        ...
        if let Some((dir, entry)) = pos {
            ...
            if let Some(dir) = regime_signal(d.zscore[i]) {
                total_entries += 1;
                let (allowed, blocked) = btc_z_confirmed(btc_z, dir, cfg);
                if blocked { total_blocked += 1; continue; }
                if allowed {
                    if i + 1 < n {
...
pub fn run(shutdown: Arc<AtomicBool>) {
    eprintln!("RUN78 — Cross-Coin Z-Score Confirmation\n");
    ...
    let results: Vec<ConfigResult> = grid.par_iter().map(|cfg| {
        ...
        let coin_results: Vec<CoinResult> = coin_data.iter().map(|d| {
            let (pnl, wins, losses, flats, blocked, entries) = simulate(d, btc_zscore, cfg);
    ...
    println!("\n=== RUN78 Cross-Coin Z-Score Confirmation Results ===");
    ...
    let notes = format!("RUN78 cross-coin Z confirmation. {} configs. Baseline PnL={:.2}. Best: {} (PnL={:.2}, Δ={:+.2})",
    ...
    std::fs::write("/home/scamarena/ProjectCoin/run78_1_results.json", &serde_json::to_string_pretty(&output).unwrap()).ok();
    eprintln!("\nSaved → run78_1_results.json");
}
