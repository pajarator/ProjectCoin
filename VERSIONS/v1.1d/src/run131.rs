...
///
/// Grid: VWRSI_LONG_MAX [30, 35, 40] × VWRSI_SHORT_MIN [60, 65, 70]
/// 9 configs × 18 coins = 162 simulations (parallel per coin)
///
/// Run: cargo run --release --features run131 -- --run131
...
fn build_grid() -> Vec<VwrsiCfg> {
    let mut grid = vec![VwrsiCfg { long_max: 0.0, short_min: 100.0, is_baseline: true }];
    for &lm in &[30.0f64, 35.0, 40.0] {
        for &sm in &[60.0f64, 65.0, 70.0] {
            grid.push(VwrsiCfg { long_max: lm, short_min: sm, is_baseline: false });
        }
    }
    grid
}
...
fn compute_vwrsi(closes: &[f64], volumes: &[f64], period: usize) -> Vec<f64> {
    let n = closes.len();
    let mut vwrsi = vec![f64::NAN; n];
    if n < period + 1 { return vwrsi; }
    ...
    for i in 1..=period {
        let delta = closes[i] - closes[i - 1];
        let vol = volumes[i];
        if delta > 0.0 { pos_sum += vol * delta; }
        else { neg_sum += vol * (-delta); }
    }
    vwrsi[period] = if neg_sum == 0.0 { 100.0 } else { 100.0 - (100.0 / (1.0 + pos_sum / neg_sum)) };
    ...
    for i in period..n {
        ...
        let gain = if delta > 0.0 { delta } else { 0.0 };
        let loss = if delta < 0.0 { -delta } else { 0.0 };
        pos_sum = (pos_sum * (period - 1) as f64 + vol * gain) / period as f64;
        neg_sum = (neg_sum * (period - 1) as f64 + vol * loss) / period as f64;
        vwrsi[i] = if neg_sum == 0.0 { 100.0 } else { 100.0 - (100.0 / (1.0 + pos_sum / neg_sum)) };
    }
    vwrsi
}
...
fn load_15m(coin: &str) -> Option<CoinData> {
    ...
    let vwrsi = compute_vwrsi(&closes, &volumes, 14);

    Some(CoinData { name: coin.to_string(), opens, closes, volumes, zscore, rsi, vwrsi })
}

fn simulate(d: &CoinData, cfg: &VwrsiCfg) -> CoinResult {
    let n = d.closes.len();
        ...
        if let Some((dir, entry, entry_bar)) = pos.as_mut() {
            ...
            if let Some(dir) = regime_signal(d.zscore[i]) {
                if cfg.is_baseline {
                    ...
                    let rsi_val = d.rsi[i];
                    let vwrsi_val = d.vwrsi[i];
                    let passes_filter = if rsi_val.is_nan() || vwrsi_val.is_nan() {
                        true
                    } else if dir == 1 {
                        // LONG: RSI < 40 AND VWRSI < long_max (volume-confirmed oversold)
                        rsi_val < 40.0 && vwrsi_val < cfg.long_max
                    } else {
                        // SHORT: RSI > 60 AND VWRSI > short_min (volume-confirmed overbought)
                        rsi_val > 60.0 && vwrsi_val > cfg.short_min
                    };
...
pub fn run(shutdown: Arc<AtomicBool>) {
    eprintln!("RUN131 — Volume-Weighted RSI Confirmation Grid Search\n");
    ...
    println!("\n=== RUN131 Volume-Weighted RSI Confirmation Results ===");
    ...
    let notes = format!("RUN131 VWRSI confirmation filter. {} configs. Baseline PnL={:.2}. Best: {} (PnL={:.2}, Δ={:+.2})",
    ...
    std::fs::write("/home/scamarena/ProjectCoin/run131_1_results.json", &serde_json::to_string_pretty(&output).unwrap()).ok();
    eprintln!("\nSaved → run131_1_results.json");
}
