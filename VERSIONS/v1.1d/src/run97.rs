/// RUN97 — BB Width Scalp Gate: Suppress Scalp Entries When Bollinger Band Width Is Low
///
/// Grid: BB_WIDTH_MIN [0.50, 0.60, 0.70, 0.80] + baseline
/// 5 configs × 18 coins = 90 simulations (1m data, 5-month)
///
/// Run: cargo run --release --features run97 -- --run97
...
impl BbGateCfg {
    fn label(&self) -> String {
        ...
        else { format!("BB{:.2}", self.width_min) }
    }
}

fn build_grid() -> Vec<BbGateCfg> {
    let mut grid = vec![BbGateCfg { width_min: 999.0, is_baseline: true }];
    for &wm in &[0.50, 0.60, 0.70, 0.80] {
        grid.push(BbGateCfg { width_min: wm, is_baseline: false });
    }
    grid
}
...
fn simulate(d: &CoinData1m, cfg: &BbGateCfg) -> CoinResult {
    let n = d.close.len();
    ...
    let mut bb_ratio_sum_blocked = 0.0f64;
    ...
    for i in 40..n {
        if let Some(p) = pos.as_mut() {
            ...
            if let Some(dir) = scalp_signal(d, i) {
                ...
                if !cfg.is_baseline && bb_ratio < cfg.width_min {
                    blocked += 1;
                    bb_ratio_sum_blocked += bb_ratio;
                } else {
                    if i + 1 < n {
    ...
    let avg_ratio_blocked = if blocked > 0 { bb_ratio_sum_blocked / blocked as f64 } else { 0.0 };

    CoinResult { coin: d.name.clone(), pnl, trades: total_trades, wins, losses, flats, wr, pf, blocked, bb_ratio_blocked: avg_ratio_blocked }
}

pub fn run(shutdown: Arc<AtomicBool>) {
    eprintln!("RUN97 — BB Width Scalp Gate Grid Search\n");
    ...
    let all_results: Vec<ConfigResult> = grid.par_iter().map(|cfg| {
        ...
        let avg_win = SCALP_RISK * LEVERAGE * SCALP_TP * (total_wins as f64);
        let avg_loss = SCALP_RISK * LEVERAGE * SCALP_SL * (total_losses as f64);
    ...
    println!("\n=== RUN97 BB Width Scalp Gate Results ===");
    ...
    let notes = format!("RUN97 bb width scalp gate. {} configs. Baseline PnL={:.2}. Best: {} (PnL={:.2}, Δ={:+.2})",
    ...
    std::fs::write("/home/scamarena/ProjectCoin/run97_1_results.json", &serde_json::to_string_pretty(&output).unwrap()).ok();
    eprintln!("\nSaved → run97_1_results.json");
}
