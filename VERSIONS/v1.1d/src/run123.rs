/// RUN123 — Accumulation/Distribution Line Divergence: Institutional Money Flow Confirmation
///
/// Grid: AD_SLOPE_PERIOD [3, 5, 10] × AD_SLOPE_MIN [0.00001, 0.0001, 0.001]
/// 9 configs × 18 coins = 162 simulations (parallel per coin)
///
/// Run: cargo run --release --features run123 -- --run123
...
impl AdCfg {
    fn label(&self) -> String {
        if self.is_baseline { "BASELINE".to_string() }
            ...
            format!("SP{}_SM{}", self.slope_period, self.slope_min)
        }
    }
}

fn build_grid() -> Vec<AdCfg> {
    let mut grid = vec![AdCfg { slope_period: 0, slope_min: 0.0, is_baseline: true }];
    for &sp in &[3usize, 5, 10] {
        for &sm in &[0.00001f64, 0.0001, 0.001] {
            grid.push(AdCfg { slope_period: sp, slope_min: sm, is_baseline: false });
        }
    }
    grid
}
...
fn load_15m(coin: &str) -> Option<CoinData> {
    ...
    // Compute A/D Line: cumulative money flow
    let mut ad_line = vec![f64::NAN; n];
    ad_line[0] = 0.0;
    for i in 1..n {
        let high = highs[i];
        let low = lows[i];
        ...
        if range > 1e-10 {
            ...
            let mfvol = mfm * vol;
            ad_line[i] = ad_line[i-1] + mfvol;
        } else {
            ad_line[i] = ad_line[i-1];
        }
    }

    Some(CoinData { name: coin.to_string(), opens, closes, highs, lows, volumes, zscore, ad_line })
}

fn simulate(d: &CoinData, cfg: &AdCfg) -> CoinResult {
    let n = d.closes.len();
        ...
        if let Some((dir, entry, entry_bar)) = pos.as_mut() {
            ...
            if let Some(dir) = regime_signal(d.zscore[i]) {
                if cfg.is_baseline {
                    ...
                    let slope_period = cfg.slope_period;
                    let ad_now = d.ad_line[i];
                    let ad_prior = if i >= slope_period { d.ad_line[i - slope_period] } else { ad_now };
                    let slope = (ad_now - ad_prior) / slope_period as f64;

                    let passes_filter = if dir == 1 {
                        // LONG: require A/D slope > threshold (rising = accumulation)
                        slope > cfg.slope_min
                    } else {
                        // SHORT: require A/D slope < -threshold (falling = distribution)
                        slope < -cfg.slope_min
                    };
...
pub fn run(shutdown: Arc<AtomicBool>) {
    eprintln!("RUN123 — A/D Line Divergence Grid Search\n");
    ...
    println!("\n=== RUN123 A/D Line Divergence Results ===");
    ...
    let notes = format!("RUN123 A/D Line filter. {} configs. Baseline PnL={:.2}. Best: {} (PnL={:.2}, Δ={:+.2})",
    ...
    std::fs::write("/home/scamarena/ProjectCoin/run123_1_results.json", &serde_json::to_string_pretty(&output).unwrap()).ok();
    eprintln!("\nSaved → run123_1_results.json");
}
