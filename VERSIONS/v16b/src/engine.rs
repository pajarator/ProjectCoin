...
pub fn check_exit(state: &mut SharedState, ci: usize) -> bool {
    ...
    if is_scalp {
        ...
        // RUN35: Breakeven stop activation — once profit reaches threshold, SL moves to entry
        if pnl >= config::SCALP_BE_ACTIVATION {
            if let Some(ref mut p) = state.coins[ci].pos {
                if p.be_active != Some(true) {
                    p.be_active = Some(true);
                    let name = state.coins[ci].name;
                    state.log(format!("BE_ACT {} [SCALP] @ {} | pnl:{:+.3}%", name, crate::state::fmt_price(check_price), pnl * 100.0));
                }
            }
        }

        let be_active = state.coins[ci].pos.as_ref().map(|p| p.be_active == Some(true)).unwrap_or(false);
        let eff_sl = if be_active { 0.0 } else { config::SCALP_SL };

        // RUN35: SL check (breakeven or normal)
        if pnl <= -eff_sl {
            let reason = if be_active { "BE" } else { "SL" };
            close_position(state, ci, check_price, reason, TradeType::Scalp);
            return true;
        }

        // RUN35: Stoch_50 exit — close when stoch_k crosses 50 while profitable
        if pnl > 0.0 {
            if let Some(ref ind_1m) = state.coins[ci].ind_1m {
                let stoch_k = ind_1m.stoch_k;
                let stoch_exit = if pos.dir == "long" {
                    stoch_k >= config::SCALP_STOCH_EXIT
                } else {
                    stoch_k <= (100.0 - config::SCALP_STOCH_EXIT)
                };
                if stoch_exit {
                    close_position(state, ci, check_price, "STOCH50", TradeType::Scalp);
                    return true;
                }
            }
        }
