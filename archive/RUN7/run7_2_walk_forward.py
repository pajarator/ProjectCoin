#!/usr/bin/env python3
"""
RUN7.2 - Walk-Forward Validation of Unified Stop Loss
...
Train: find best SL params per coin (initial_sl, trail_mode, activation, distance).
Test: apply OOS. Compare universal vs per-coin vs current baseline (sl=0.5%, none).
"""
import pandas as pd
import numpy as np
import json
...
CHECKPOINT_FILE = '/home/scamarena/ProjectCoin/run7_2_checkpoint.json'
RESULTS_FILE = '/home/scamarena/ProjectCoin/run7_2_results.json'
    ...
    for idx, row in df.iterrows():
        ...
        if position == 'long':
            ...
            if price_pnl <= -initial_sl:
                ...
                trades.append({'pnl_pct': -initial_sl * LEVERAGE * 100, 'type': 'loss', 'reason': 'SL'})
                exited = True
            ...
            if not exited and trail_mode != 'none':
                ...
                if peak_pct >= trail_activation:
                    if trail_mode == 'breakeven' and price <= entry_price:
                        ...
                        trades.append({'pnl_pct': price_pnl * LEVERAGE * 100,
...
def main():
    ...
    print("=" * 90)
    print("RUN7.2 - WALK-FORWARD VALIDATION OF UNIFIED STOP LOSS")
    print("=" * 90)
    print(f"Train: 2 months | Test: 1 month | 3 windows")
    print(f"SL combos: {len(COMBOS)} (none + breakeven + trail)")
    print("=" * 90)
    ...
    # Load RUN7.1 results for universal params
    ...
    if os.path.exists(r71_file):
        with open(r71_file, 'r') as f:
            r71 = json.load(f)
        if 'best_overall' in r71 and r71['best_overall']:
            universal_params = r71['best_overall']
            print(f"Loaded universal params: {universal_params['trail_mode']} "
                  f"sl={universal_params['initial_sl']} act={universal_params['trail_activation']} "
                  f"dist={universal_params['trail_distance']}")
    else:
        print("WARNING: run7_1_results.json not found, will search all combos in train")
    ...
    for coin in COINS:
        ...
        for w in WINDOWS:
            ...
            # === TRAIN: find best SL combo ===
            best_train_score = -999
            best_train_combo = None
            best_train_result = None

            for tmode, sl, act, dist in COMBOS:
                r = run_backtest(
                    train_df, long_strat, short_strat, iso_short_strat,
                    train_breadth, train_avg_z, train_avg_rsi, train_btc_z,
                    initial_sl=sl, trail_mode=tmode, trail_activation=act, trail_distance=dist)
                if r and r['trades'] >= 3:
                    score = r['pf'] * (r['wr'] / 100) ** 0.5
                    if score > best_train_score:
                        best_train_score = score
                        best_train_combo = (tmode, sl, act, dist)
                        best_train_result = r

            # === TEST: per-coin best ===
            test_percoin = None
            if best_train_combo:
                tmode, sl, act, dist = best_train_combo
            ...
            if universal_params:
                test_universal = run_backtest(
                    test_df, long_strat, short_strat, iso_short_strat,
                    test_breadth, test_avg_z, test_avg_rsi, test_btc_z,
                    initial_sl=universal_params['initial_sl'],
                    trail_mode=universal_params['trail_mode'],
                    trail_activation=universal_params['trail_activation'],
                    trail_distance=universal_params['trail_distance'])
    ...
    print(f"  Baseline (sl=0.5%, none): Test PF {avg_test_base:.2f}")
    ...
    # Is optimized SL better than baseline?
    best_test = max(avg_test_univ, avg_test_pc)
    if best_test > avg_test_base:
        print(f"\n  VERDICT: Optimized SL ({best_test:.2f}) BEATS baseline ({avg_test_base:.2f})")
    else:
        print(f"\n  VERDICT: Baseline SL ({avg_test_base:.2f}) is already optimal or near-optimal")
