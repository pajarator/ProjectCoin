//! RUN13.2 — Walk-Forward Validation
//! 3-window walk-forward for RUN13.1 winners
//! Test across ALL 19 coins

use run13_lib::indicators;
...
const RESULTS_FILE: &str = "/home/scamarena/ProjectCoin/run13/run13_2_results.json";
...
fn wf_configs() -> Vec<StratConfig> {
    // Re-test all RUN13 strategies with representative params
    ...
    for &wick in &[0.6, 0.7] {
        for &body in &[0.2, 0.3] {
            for &z in &[0.0, -0.5, -1.0] {
                cfgs.push(StratConfig { name: "pin_bar", p1: wick, p2: body, p3: 0.0, z_filter: z });
            }
        }
    }
    ...
    for &wick in &[0.6, 0.7] {
        for &z in &[0.0, -0.5, -1.0] {
            cfgs.push(StratConfig { name: "hammer", p1: wick, p2: 0.0, p3: 0.0, z_filter: z });
        }
    }

    // Engulfing
    for &z in &[0.0, -0.5, -1.0] {
        cfgs.push(StratConfig { name: "engulfing", p1: 0.0, p2: 0.0, p3: 0.0, z_filter: z });
    }

    // QQE
    for &mult in &[0.0, 1.0] {
        for &z in &[0.0, -0.5, -1.0] {
            cfgs.push(StratConfig { name: "qqe", p1: mult, p2: 0.0, p3: 0.0, z_filter: z });
        }
    }
    ...
    for g in 0..4 {
        for &z in &[0.0, -0.5, -1.0] {
            cfgs.push(StratConfig { name: "laguerre_rsi", p1: g as f64, p2: 0.0, p3: 0.0, z_filter: z });
        }
    }
    ...
    for &t in &[26.5, 27.0] {
        for &z in &[0.0, -0.5, -1.0] {
            cfgs.push(StratConfig { name: "mass_index", p1: t, p2: 0.0, p3: 0.0, z_filter: z });
        }
    }

    // KST
    for &z in &[0.0, -0.5, -1.0] {
        cfgs.push(StratConfig { name: "kst_cross", p1: 0.0, p2: 0.0, p3: 0.0, z_filter: z });
    }
    ...
    for &period in &[0.0, 1.0] {
        for &typ in &[0.0, 1.0] {
            for &z in &[0.0, -0.5, -1.0] {
                cfgs.push(StratConfig { name: "dema_tema", p1: period, p2: typ, p3: 0.0, z_filter: z });
            }
        }
    }
    ...
    for &v in &[0.0, 1.0] {
        for &z in &[0.0, -0.5, -1.0] {
            cfgs.push(StratConfig { name: "parabolic_sar", p1: v, p2: 0.0, p3: 0.0, z_filter: z });
        }
    }

    // Ichimoku
    for &z in &[0.0, -0.5, -1.0] {
        cfgs.push(StratConfig { name: "ichimoku", p1: 0.0, p2: 0.0, p3: 0.0, z_filter: z });
    }

    // Kalman Filter
    for q in 0..3 {
        for &z in &[0.0, -0.5, -1.0] {
            cfgs.push(StratConfig { name: "kalman_filter", p1: q as f64, p2: 0.0, p3: 0.0, z_filter: z });
        }
    }
...
fn main() {
    println!("=== RUN13.2 — Walk-Forward Validation ===");
