/// RUN122 — Elder Ray Bull Power Confirmation: Entry Filter Based on High/Low vs EMA
///
/// Grid: EMA_PERIOD [9, 13, 21] × BULL_CONFIRM [true, false] × BEAR_CONFIRM [true, false]
/// 12 configs × 18 coins = 216 simulations (parallel per coin)
///
/// Run: cargo run --release --features run122 -- --run122
...
impl ElderCfg {
    fn label(&self) -> String {
        if self.is_baseline { "BASELINE".to_string() }
            ...
            let bc = if self.bull_confirm { "B1" } else { "B0" };
            let be = if self.bear_confirm { "E1" } else { "E0" };
            format!("EP{}_{}_{}", self.ema_period, bc, be)
        }
    }
}

fn build_grid() -> Vec<ElderCfg> {
    let mut grid = vec![ElderCfg { ema_period: 0, bull_confirm: false, bear_confirm: false, is_baseline: true }];
    for &ep in &[9usize, 13, 21] {
        for &bc in &[true, false] {
            for &be in &[true, false] {
                grid.push(ElderCfg { ema_period: ep, bull_confirm: bc, bear_confirm: be, is_baseline: false });
            }
        }
    }
    grid
...
fn load_15m(coin: &str, ema_period: usize) -> Option<CoinData> {
    ...
    Some(CoinData { name: coin.to_string(), opens, closes, highs, lows, zscore, bull_power, bear_power })
}

fn simulate(d: &CoinData, cfg: &ElderCfg) -> CoinResult {
    let n = d.closes.len();
            ...
            if let Some(dir) = regime_signal(d.zscore[i]) {
                if cfg.is_baseline {
                    ...
                    let passes_filter = if dir == 1 {
                        // LONG: require bull power < 0 AND rising
                        if !cfg.bull_confirm {
                            true
                        } else {
                            let bp = d.bull_power[i];
                            let bp_prior = d.bull_power[i-1];
                            if bp.is_nan() || bp_prior.is_nan() {
                                true
                            } else {
                                bp < 0.0 && bp > bp_prior
                            }
                        ...
                        // SHORT: require bear power > 0 AND falling
                        if !cfg.bear_confirm {
                            true
                        } else {
                            let bp = d.bear_power[i];
                            let bp_prior = d.bear_power[i-1];
                            if bp.is_nan() || bp_prior.is_nan() {
                                true
                            } else {
                                bp > 0.0 && bp < bp_prior
...
pub fn run(shutdown: Arc<AtomicBool>) {
    eprintln!("RUN122 — Elder Ray Bull Power Confirmation Grid Search\n");
    let grid = build_grid();
    eprintln!("Grid: {} configs × {} coins = {} simulations", grid.len(), N_COINS, grid.len() * N_COINS);

    // Precompute data for each EMA period
    ...
    let all_results: Vec<ConfigResult> = grid.par_iter().map(|cfg| {
        ...
        // Load data for this EMA period
        let ema_period = if cfg.is_baseline { 13 } else { cfg.ema_period };
        let mut raw_data: Vec<Option<CoinData>> = Vec::new();
        for &name in &COIN_NAMES {
            raw_data.push(load_15m(name, ema_period));
        }
    ...
    println!("\n=== RUN122 Elder Ray Bull Power Confirmation Results ===");
    ...
    let notes = format!("RUN122 Elder Ray filter. {} configs. Baseline PnL={:.2}. Best: {} (PnL={:.2}, Δ={:+.2})",
    ...
    std::fs::write("/home/scamarena/ProjectCoin/run122_1_results.json", &serde_json::to_string_pretty(&output).unwrap()).ok();
    eprintln!("\nSaved → run122_1_results.json");
}
