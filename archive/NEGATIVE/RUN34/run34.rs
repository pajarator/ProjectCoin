...
///
/// Hypothesis: ISO short entries bleed capital through repeated SL hits when
/// a coin is trending up and the system mistakes momentum for coin-specific
/// overbought conditions. Two independent mechanisms are tested:
///
///   A. Circuit breaker — after N consecutive ISO short SLs on the same coin,
///      suppress ISO shorts for that coin for X bars.
///   B. ISO-specific SL widening — use a wider (or no) stop loss on ISO shorts
///      so mean-reversion signals have time to work.
///
/// Grid: 15 combos (1 baseline + 6 CB + 4 SL + 3 combined + 1 kill-ISO)
/// Decision gate: ΔP&L > 0 AND ΔWR > 0
///
/// Run: cargo run --release --features run34 -- --run34
...
const SL: f64          = 0.003;   // 0.3% regime stop loss
...
struct GridCfg {
    label:   &'static str,
    cb_n:    u32,    // circuit breaker: trigger after N consecutive ISO SLs (0=disabled)
    cb_cd:   usize,  // circuit breaker: cooldown bars when triggered
    iso_sl:  f64,    // ISO-specific SL (f64::MAX = signal-only, 0.0 = kill ISO)
    iso_off: bool,   // true = disable all ISO shorts
}

const GRID: [GridCfg; 15] = [
    // 0. Baseline
    GridCfg { label: "baseline",          cb_n: 0, cb_cd: 0,  iso_sl: 0.003, iso_off: false },
    // A. Circuit breaker variants
    GridCfg { label: "CB(2,4)",           cb_n: 2, cb_cd: 4,  iso_sl: 0.003, iso_off: false },
    GridCfg { label: "CB(2,8)",           cb_n: 2, cb_cd: 8,  iso_sl: 0.003, iso_off: false },
    GridCfg { label: "CB(2,16)",          cb_n: 2, cb_cd: 16, iso_sl: 0.003, iso_off: false },
    GridCfg { label: "CB(3,4)",           cb_n: 3, cb_cd: 4,  iso_sl: 0.003, iso_off: false },
    GridCfg { label: "CB(3,8)",           cb_n: 3, cb_cd: 8,  iso_sl: 0.003, iso_off: false },
    GridCfg { label: "CB(3,16)",          cb_n: 3, cb_cd: 16, iso_sl: 0.003, iso_off: false },
    // B. ISO SL widening
    GridCfg { label: "ISO_SL=0.5%",       cb_n: 0, cb_cd: 0, iso_sl: 0.005, iso_off: false },
    GridCfg { label: "ISO_SL=0.8%",       cb_n: 0, cb_cd: 0, iso_sl: 0.008, iso_off: false },
    GridCfg { label: "ISO_SL=1.5%",       cb_n: 0, cb_cd: 0, iso_sl: 0.015, iso_off: false },
    GridCfg { label: "ISO_SL=signal",     cb_n: 0, cb_cd: 0, iso_sl: f64::MAX, iso_off: false },
    // C. Combined (CB + SL)
    GridCfg { label: "CB(2,8)+SL=0.5%",   cb_n: 2, cb_cd: 8, iso_sl: 0.005, iso_off: false },
    GridCfg { label: "CB(2,8)+SL=0.8%",   cb_n: 2, cb_cd: 8, iso_sl: 0.008, iso_off: false },
    GridCfg { label: "CB(2,8)+SL=signal", cb_n: 2, cb_cd: 8, iso_sl: f64::MAX, iso_off: false },
    // D. Kill ISO entirely
    GridCfg { label: "no-ISO",            cb_n: 0, cb_cd: 0, iso_sl: 0.003, iso_off: true },
];
const N_GRID: usize = 15;
...
// ── Rolling helpers (identical to run32) ──────────────────────────────────────
...
fn kalman_all(c: &[f64], q: f64, r: f64) -> Vec<(f64, f64)> {
    ...
    let mut out = vec![(f64::NAN, f64::NAN); n];
    if n == 0 { return out; }
    let mut x = c[0]; let mut p = 1.0_f64;
    out[0] = (x, p);
    for i in 1..n {
        let p_pred = p + q;
...
fn kst_all(c: &[f64]) -> Vec<(f64, f64, f64, f64)> {
    ...
    for i in 1..n { out[i] = (kst_v[i], sig_v[i], kst_v[i-1], sig_v[i-1]); }
    out
}
...
fn compute_indicators(bars: &[Bar], is_dash: bool) -> Vec<Ind> {
    ...
    let bb_width_raw: Vec<f64> = (0..n).map(|i| if !std20[i].is_nan() { 4.0*std20[i] } else { f64::NAN }).collect();
...
fn load_15m(coin: &str) -> (Vec<Bar>, Vec<u32>) {
    let path = format!(
        "/home/scamarena/ProjectCoin/data_cache/{}_USDT_15m_1year.csv", coin
    );
...
struct Position {
    ...
    entry:     f64,
    bars_held: usize,
    is_iso:    bool,   // true = this was an ISO short entry
}
...
struct SimResult {
    ...
    iso_pnl:       f64,
    non_iso_pnl:   f64,
    cb_blocked:    usize,         // trades blocked by circuit breaker
    coin_trades:   Vec<usize>,
    coin_wins:     Vec<usize>,
    coin_pnl:      Vec<f64>,
    coin_iso_trades: Vec<usize>,
    coin_iso_sl:     Vec<usize>,  // ISO SL count per coin
    coin_max_consec: Vec<u32>,    // max consecutive ISO SLs observed per coin
}
...
fn simulate(
    ...
    let mut iso_trades   = 0usize;
    let mut iso_wins     = 0usize;
    let mut iso_pnl      = 0.0f64;
    let mut non_iso_pnl  = 0.0f64;
    let mut cb_blocked   = 0usize;

    let mut coin_trades     = vec![0usize; N_COINS];
    let mut coin_wins       = vec![0usize; N_COINS];
    let mut coin_pnl        = vec![0.0f64; N_COINS];
    let mut coin_iso_trades = vec![0usize; N_COINS];
    let mut coin_iso_sl     = vec![0usize; N_COINS];
    let mut coin_max_consec = vec![0u32; N_COINS];
    ...
    // Circuit breaker state: per-coin consecutive ISO short SL counter
    let mut consec_iso_sl: Vec<u32>   = vec![0; N_COINS];
    let mut iso_cd:        Vec<usize> = vec![0; N_COINS]; // circuit breaker cooldown
    ...
    for bar_i in 1..n_bars {
                ...
                if let Some(ref pos) = positions[ci] {
                    ...
                    *daily_pnl.entry(current_day).or_insert(0.0) += trade_pnl;
                    ...
                    if pos.is_iso {
                        iso_trades += 1;
                        iso_pnl += trade_pnl;
                        coin_iso_trades[ci] += 1;
                        if pf > 0.0 { iso_wins += 1; }
                    } else {
                        non_iso_pnl += trade_pnl;
                    }
                    positions[ci] = None;
                }
        ...
        for ci in 0..N_COINS {
            if let Some(ref pos) = positions[ci] {
                ...
                let active_sl = if pos.is_iso { cfg.iso_sl } else { SL };

                let mut closed = false;
                let mut pf     = 0.0;
                let mut was_sl = false;

                if active_sl < f64::MAX && pnl <= -active_sl {
                    pf = -(active_sl + FEE + SLIP);
                    closed = true;
                    was_sl = true;
                    ...
                    *daily_pnl.entry(bar_day).or_insert(0.0) += trade_pnl;
                    ...
                    if was_iso {
                        ...
                        if was_sl {
                            ...
                            consec_iso_sl[ci] += 1;
                            coin_iso_sl[ci] += 1;
                            if consec_iso_sl[ci] > coin_max_consec[ci] {
                                coin_max_consec[ci] = consec_iso_sl[ci];
                            }
                            // Trigger circuit breaker if threshold reached
                            if cfg.cb_n > 0 && consec_iso_sl[ci] >= cfg.cb_n {
                                iso_cd[ci] = cfg.cb_cd;
                            }
                        } else {
                            consec_iso_sl[ci] = 0; // non-SL exit resets streak
                        }
                    } else {
                        non_iso_pnl += trade_pnl;
                    }
        ...
        for ci in 0..N_COINS {
            ...
            let ind = &all_inds[ci][bar_i];
            if !ind.valid { continue; }
            let coin_cfg = &COIN_CFGS[ci];
            if is_squeeze(ind) { continue; }

            let entry_price = all_bars[ci][bar_i + 1].o;
            if entry_price <= 0.0 { continue; }

            // Determine if ISO short is allowed for this coin right now
            let iso_allowed = !cfg.iso_off && iso_cd[ci] == 0;

            let result: Option<(Dir, bool)> = match mode {
                Mode::Long => {
                    if long_signal(ind, coin_cfg.long) {
                        Some((Dir::Long, false))
                    } else if comp_signal(ind, coin_cfg.comp, coin_cfg.comp_z) {
                        Some((Dir::Long, false))
                    } else if iso_allowed && iso_signal(ind, coin_cfg.iso, avg_z, avg_rsi, btc_z) {
                        Some((Dir::Short, true))
                    } else {
                        if !iso_allowed && iso_signal(ind, coin_cfg.iso, avg_z, avg_rsi, btc_z) {
                            cb_blocked += 1;
                        }
                        None
                    }
                ...
                Mode::Short => {
                    if short_signal(ind, coin_cfg.short) {
                        Some((Dir::Short, false))
                    } else {
                        None
                    }
                }
            ...
            if let Some((d, is_iso)) = result {
                positions[ci] = Some(Position {
                    dir: d, entry: entry_price, bars_held: 0, is_iso,
                });
            }
        }
    }
    ...
    SimResult {
        ...
        iso_trades, iso_wins, iso_pnl, non_iso_pnl,
        cb_blocked,
        coin_trades, coin_wins, coin_pnl,
        coin_iso_trades, coin_iso_sl, coin_max_consec,
    }
}
...
pub fn run(shutdown: Arc<AtomicBool>) {
    eprintln!("RUN34 — ISO Short Drawdown Mitigation");
    eprintln!("Grid: {} combos (baseline + circuit breakers + SL widening + combos + kill-ISO)", N_GRID);
    eprintln!("Decision gate: ΔP&L > 0 AND ΔWR > 0");
    ...
    eprintln!("Running {} grid combos in parallel...", N_GRID);

    let sims: Vec<SimResult> = GRID.par_iter()
        .map(|cfg| simulate(&all_inds, &all_bars, &all_days, &ordered_days, cfg))
        .collect();
    ...
    let mut stats: Vec<ComboStats> = GRID.iter().zip(sims.iter()).enumerate().map(|(idx, (cfg, sim))| {
        ...
        let iso_wr = if sim.iso_trades > 0 {
            sim.iso_wins as f64 / sim.iso_trades as f64 * 100.0
        } else { 0.0 };
        let delta = total - baseline_total;
        let gate  = delta > 0.0 && wr > baseline_wr;
        ComboStats {
            idx, label: cfg.label,
            total_pnl: total, delta, trades: sim.total_trades,
            wr_pct: wr, iso_trades: sim.iso_trades, iso_wins: sim.iso_wins,
            iso_wr_pct: iso_wr, iso_pnl: sim.iso_pnl, non_iso_pnl: sim.non_iso_pnl,
            cb_blocked: sim.cb_blocked,
            pos_days, pos_days_pct: pos_pct, meets_gate: gate,
        }
    }).collect();
    ...
    println!("\n{:-<130}", "");
    println!("{:<22} {:>9}  {:>8}  {:>6}  {:>5}  {:>6}  {:>5}  {:>9}  {:>10}  {:>7}  {:>5}  {:>6}",
        "Config", "TotalPnL", "ΔBase", "Trades", "WR%", "ISOtrd", "IWR%", "ISO_PnL", "nonISO_PnL", "Blocked", "+Days", "+Day%");
    println!("{:-<130}", "");

    let print_row = |label: &str, s: &ComboStats, marker: &str| {
        println!("{}{:<21} {:>+9.2}  {:>+8.2}  {:>6}  {:>4.1}%  {:>6}  {:>4.1}%  {:>+9.2}  {:>+10.2}  {:>7}  {:>5}  {:>5.1}%",
            marker, label,
            s.total_pnl, s.delta, s.trades, s.wr_pct,
            s.iso_trades, s.iso_wr_pct, s.iso_pnl, s.non_iso_pnl,
            s.cb_blocked, s.pos_days, s.pos_days_pct);
    };
    ...
    println!("{:-<130}", "");
    println!("* = meets decision gate (ΔP&L > 0 AND ΔWR > 0)   {} of {} combos pass", n_gate, stats.len());

    // ── Per-coin ISO short diagnostics (baseline) ────────────────────────────
    println!("\n── Per-coin ISO Short Diagnostics (baseline) ──────────────────────────────────");
    println!("{:<6} {:>6}  {:>6}  {:>6}  {:>6}  {:>9}  {:>9}",
        "Coin", "ISOtrd", "ISO_SL", "MaxSeq", "ISO_WR%", "ISO_PnL", "Total_PnL");
    println!("{:-<66}", "");
    let base_sim = &sims[0];
    let mut ci_order: Vec<usize> = (0..N_COINS).collect();
    ci_order.sort_by(|&a, &b| {
        let pa = base_sim.coin_iso_sl[a] as f64;
        let pb = base_sim.coin_iso_sl[b] as f64;
        pb.partial_cmp(&pa).unwrap_or(std::cmp::Ordering::Equal)
    });
    for ci in &ci_order {
        let iso_wr = if base_sim.coin_iso_trades[*ci] > 0 {
            // approximate: we don't track per-coin ISO wins in baseline, so use overall ratio
            // Actually we track coin_iso_sl, so wins = iso_trades - iso_sl (approximate)
            let wins = base_sim.coin_iso_trades[*ci].saturating_sub(base_sim.coin_iso_sl[*ci]);
            wins as f64 / base_sim.coin_iso_trades[*ci] as f64 * 100.0
        } else { 0.0 };
        // For ISO P&L per coin, we'd need separate tracking. Use total coin pnl as context.
        println!("{:<6} {:>6}  {:>6}  {:>6}  {:>5.1}%  {:>9}  {:>+9.2}",
            COIN_CFGS[*ci].name,
            base_sim.coin_iso_trades[*ci],
            base_sim.coin_iso_sl[*ci],
            base_sim.coin_max_consec[*ci],
            iso_wr,
            "—", // ISO P&L per coin not tracked separately
            base_sim.coin_pnl[*ci]);
    }

    // ── Per-coin breakdown for top combos ─────────────────────────────────────
    ...
    if !top.is_empty() {
        ...
        for s in &top {
            let sim = &sims[s.idx];
            println!("\n  [{}]  P&L={:+.2}  ΔBase={:+.2}  ISOtrd={}  Blocked={}",
                s.label, s.total_pnl, s.delta, s.iso_trades, s.cb_blocked);
            println!("  {:<6} {:>6}  {:>5}  {:>5}  {:>6}  {:>6}  {:>6}  {:>9}  {:>9}",
                "Coin", "Trades", "Wins", "WR%", "ISOtrd", "ISO_SL", "MaxSeq", "P&L", "ΔBase");
            let mut ci_order: Vec<usize> = (0..N_COINS).collect();
            ci_order.sort_by(|&a, &b| {
                let da = sim.coin_pnl[a] - baseline_coin_pnl[a];
                let db = sim.coin_pnl[b] - baseline_coin_pnl[b];
                db.partial_cmp(&da).unwrap_or(std::cmp::Ordering::Equal)
            });
            for ci in ci_order {
                let wr = if sim.coin_trades[ci] > 0 {
                    sim.coin_wins[ci] as f64 / sim.coin_trades[ci] as f64 * 100.0
                } else { 0.0 };
                println!("  {:<6} {:>6}  {:>5}  {:>4.1}%  {:>6}  {:>6}  {:>6}  {:>+9.2}  {:>+9.2}",
                    COIN_CFGS[ci].name,
                    sim.coin_trades[ci], sim.coin_wins[ci], wr,
                    sim.coin_iso_trades[ci], sim.coin_iso_sl[ci], sim.coin_max_consec[ci],
                    sim.coin_pnl[ci], sim.coin_pnl[ci] - baseline_coin_pnl[ci]);
            }
        }
    }
    ...
    // Helper to build per_coin for a sim
    let build_coin_bd = |sim: &SimResult| -> Vec<CoinBreakdown> {
        (0..N_COINS).map(|ci| {
            let wr = if sim.coin_trades[ci] > 0 {
                sim.coin_wins[ci] as f64 / sim.coin_trades[ci] as f64 * 100.0
            } else { 0.0 };
            CoinBreakdown {
                ...
                trades: sim.coin_trades[ci], wins: sim.coin_wins[ci],
                wr_pct: wr, pnl: sim.coin_pnl[ci],
                iso_trades: sim.coin_iso_trades[ci],
                iso_sl: sim.coin_iso_sl[ci],
                max_consec: sim.coin_max_consec[ci],
                delta_pnl: sim.coin_pnl[ci] - baseline_coin_pnl[ci],
            }
        }).collect()
    };
    ...
    all_results.push(GridResult {
        ...
        total_pnl: baseline_stat.total_pnl, delta_baseline: 0.0,
        trades: baseline_stat.trades, win_rate_pct: baseline_stat.wr_pct,
        iso_trades: baseline_stat.iso_trades, iso_wins: baseline_stat.iso_wins,
        iso_wr_pct: baseline_stat.iso_wr_pct, iso_pnl: baseline_stat.iso_pnl,
        non_iso_pnl: baseline_stat.non_iso_pnl, cb_blocked: 0,
        positive_days: baseline_stat.pos_days, positive_days_pct: baseline_stat.pos_days_pct,
        meets_gate: false,
        per_coin: build_coin_bd(&sims[0]),
    });
    ...
    for s in &stats {
        ...
        all_results.push(GridResult {
            ...
            total_pnl: s.total_pnl, delta_baseline: s.delta,
            trades: s.trades, win_rate_pct: s.wr_pct,
            iso_trades: s.iso_trades, iso_wins: s.iso_wins,
            iso_wr_pct: s.iso_wr_pct, iso_pnl: s.iso_pnl,
            non_iso_pnl: s.non_iso_pnl, cb_blocked: s.cb_blocked,
            positive_days: s.pos_days, positive_days_pct: s.pos_days_pct,
            meets_gate: s.meets_gate,
            per_coin: build_coin_bd(sim),
        });
    }

    let output = Run34Output {
        notes: format!(
            "RUN34: ISO Short Drawdown Mitigation. {} coins, {} days. \
             $100/coin/day reset. RISK={}% LEVERAGE={}x FEE={}%/side SLIP={}%/side. \
             Tests: circuit breaker (N consec SLs → X-bar cooldown), \
             ISO-specific SL widening (0.5%, 0.8%, 1.5%, signal-only), \
             combined, and ISO kill. \
        ...
        baseline_wr,
        baseline_iso_pnl: sims[0].iso_pnl,
        decision_gate: "delta_baseline > 0 AND delta_wr > 0".to_string(),
        results: all_results,
    };

    let json = serde_json::to_string_pretty(&output).unwrap();
    let path = "/home/scamarena/ProjectCoin/run34_1_results.json";
