...
archive/
  ...
  RUN27/ — Breakout momentum rider (hard rally/crash). Result: CONDITIONALLY POSITIVE — Walk-forward (3 windows) confirms NEAR LONG (avg OOS 42.8%) and XLM LONG (avg 45.2%). DASH LONG fails walk-forward (avg 33.2%) but +25.4% PnL via R:R=2.5. XRP SHORT new positive in walk-forward (avg 52.8%). Comparison (RUN27.3): momentum alone -2% total vs COINCLAW -1512%; combined OR signal catastrophic (-7140%) — cannot merge with COINCLAW's 0.3% SL. Momentum breakout must run as independent layer with ATR stop if added to COINCLAW. No immediate changes pending COINCLAW ATR-stop integration design.
  RUN28/ — Momentum persistence classifier. Diagnoses which coins have genuine post-breakout continuation vs mean-reversion. Result: NEAR (+2.4% edge) and XLM (+5.3% strict) confirmed as persistence coins. DASH is right-tail skew (avg_fwd_ret=+0.822%) not direction persistence — different mechanism. Hard exclusions: ATOM (-12.4% edge), SHIB (-6.1%), LTC (-5.2%), ADA (-5.2%), DOT (-5.1%), BNB (-3.7%) all mean-revert after breakouts. Provides principled coin selection filter for COINCLAW momentum layer integration.
  RUN29/ — MAX_SCALP_OPENS_PER_CYCLE optimization (1-year 1m, 18 coins). Result: NEGATIVE — cap is irrelevant (avg 1.097 simultaneous signals/bar; cap=3 captures 99.9% of all signals). Deeper finding: scalp WR=10.9% vs breakeven 27.8%; strategy loses on all 18 coins over full year. v9/v11 live outperformance was a 30-min correlated-pump artifact. No COINCLAW changes.
  RUN34/ — ISO short drawdown mitigation: circuit breaker (N consec SLs → X-bar cooldown) + ISO-specific SL widening (0.5%/0.8%/1.5%/signal-only). Result: NEGATIVE — ISO shorts are the primary profit engine (+$118 of +$144 total, 82%). Every mitigation hurts P&L. Wider stops raise WR (signal-only=70%) but avg loss grows faster than avg win. Consecutive SL streaks (max 9/coin/year) are normal operating variance, not a system flaw. No COINCLAW changes.
  RUN101/ — Partial Position Split: split each regime entry into core + satellite halves with different exits. Result: NEGATIVE — Walk-forward (RUN101.2) shows Δ=0 across all windows. The simplified backtest does not implement satellite differently from core. No COINCLAW changes.
  RUN102/ — TWAP Entry Execution: accumulate positions over 4 bars, cancel if price moves ≥0.3% adversely. Result: NEGATIVE — Walk-forward (RUN102.2) shows avg Δ=-2.91, 0/3 positive windows. In-sample improvement (+553 delta) doesn't survive OOS. No COINCLAW changes.
  RUN103/ — Stochastic Extreme Exit: exit regime trades when stochastic reaches extreme (OB≥85/OS≤25) while profitable. Result: MARGINAL POSITIVE — Grid search shows +$1.76 delta (noise-level). Walk-forward not run. Not worth implementation complexity. Archived to NEGATIVE.
  RUN104/ — Volume Dry-Up Exit: exit when volume collapses below threshold while profitable. Result: MARGINAL POSITIVE — Grid search shows +$0.68 delta (noise-level). Walk-forward not run. Not worth implementation complexity. Archived to NEGATIVE.
  RUN105/ — Z-Score Persistence Filter: require z< -1.3 or z>1.3 for 2 consecutive bars before entry (vs single-bar ±2.0). Result: POSITIVE — Walk-forward (RUN105.2) confirms avg Δ=+9.73, 3/3 positive windows. Change: lower entry threshold to ±1.3σ with 2-bar persistence requirement. Net +17.5% portfolio PnL improvement.
```
