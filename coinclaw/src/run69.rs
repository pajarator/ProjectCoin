...
///
/// Grid: STREAK_COUNT [2, 3, 4, 5] × STREAK_Z_EXIT [0.3, 0.4, 0.5]
/// After N consecutive wins, next trade exits at z > -streak_z_exit (vs normal z > 0.5)
/// Total: 4 × 3 = 12 configs + baseline = 13
///
/// Run: cargo run --release --features run69 -- --run69
...
impl StreakCfg {
    fn label(&self) -> String {
        format!("SC{}_SE{:+.1}", self.streak_count, -self.streak_z_exit)
    }
}

fn build_grid() -> Vec<StreakCfg> {
    let mut grid = vec![StreakCfg { streak_count: 0, streak_z_exit: 0.5 }]; // baseline
    for &sc in &[2, 3, 4, 5] {
        for &se in &[0.3, 0.4, 0.5] {
            grid.push(StreakCfg { streak_count: sc, streak_z_exit: se });
        }
    }
    grid
}
...
fn simulate(d: &CoinData15m, cfg: StreakCfg) -> (f64, usize, usize, usize) {
    ...
    for i in 1..n {
        if let Some((dir, entry)) = pos {
            ...
            if !closed && i >= 2 {
                // Z-exit threshold depends on streak mode
                let z = d.zscore[i];
                let streak_active = cfg.streak_count > 0 && consec_wins >= cfg.streak_count;
                let z_threshold = if streak_active { cfg.streak_z_exit } else { 0.5 };

                let z_exit = if !z.is_nan() {
                    if dir == 1 && z > z_threshold { true }
                    else if dir == -1 && z < -z_threshold { true }
                    else { false }
                } else { false };

                // Also check for regime reversal
                let new_dir = regime_signal(d, i);
                let regime_exit = new_dir.is_some() && new_dir != Some(dir);

                if z_exit || regime_exit { exit_pct = pct; closed = true; }
            }

            if closed {
                let net = (bal * BASE_POSITION_SIZE * LEVERAGE) * exit_pct;
                bal += net;
                if net > 1e-10 { wins += 1; consec_wins += 1; }
                else if net < -1e-10 { losses += 1; consec_wins = 0; }
                else { flats += 1; }
                pos = None;
                cooldown = COOLDOWN;
            }
...
pub fn run(shutdown: Arc<AtomicBool>) {
    eprintln!("RUN69 — Winning Streak Profit-Taking\n");
    ...
    let results: Vec<ConfigResult> = grid.par_iter().map(|cfg| {
        if shutdown.load(Ordering::SeqCst) {
            return ConfigResult {
                ...
                total_trades: 0, pf: 0.0, is_baseline: cfg.streak_count == 0,
            };
        }
        ...
        ConfigResult {
            ...
            total_trades,
            pf,
            is_baseline: cfg.streak_count == 0,
        }
    }).collect();
    ...
    let notes = format!("RUN69 winning streak. {} configs. Baseline PnL={:.2}. Best: {} (PnL={:.2}, Δ={:+.2})",
    ...
    std::fs::write("/home/scamarena/ProjectCoin/run69_1_results.json", &serde_json::to_string_pretty(&output).unwrap()).ok();
    eprintln!("\nSaved → run69_1_results.json");
}