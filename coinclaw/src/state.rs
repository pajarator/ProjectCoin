...
pub struct CoinPersist {
    ...
    pub cooldown: u32,
    // RUN94: partial reentry tracking
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub last_entry_z: Option<f64>,
    #[serde(default)]
...
impl CoinState {
    ...
    pub fn to_persist(&self) -> CoinPersist {
        CoinPersist {
            ...
            last_entry_z: self.last_entry_z,
    ...
    pub fn load_state(&mut self) {
        ...
        for cs in &mut self.coins {
            if let Some(persisted) = map.get(cs.name) {
                ...
                cs.last_entry_z = persisted.last_entry_z;
                cs.reentry_count = persisted.reentry_count;
            }
        }
    }
