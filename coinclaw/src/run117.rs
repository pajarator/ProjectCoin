/// RUN117 — ROC Percentile Filter: Require ROC at Historical Extreme Percentile
///
/// Grid: ROC_LONG_PCT [5, 10, 20] × ROC_SHORT_PCT [5, 10, 20] × ROC_LOOKBACK [3, 5, 10]
/// 27 configs × 18 coins = 486 simulations (parallel per coin)
///
/// Run: cargo run --release --features run117 -- --run117
...
impl RocPctCfg {
    fn label(&self) -> String {
        ...
        else { format!("LP{}_SP{}_LB{}", self.lp as usize, self.sp as usize, self.lb) }
    }
}

fn build_grid() -> Vec<RocPctCfg> {
    let mut grid = vec![RocPctCfg { lp: 0.0, sp: 0.0, lb: 0, is_baseline: true }];
    for &lp in &[5.0, 10.0, 20.0] {
        for &sp in &[5.0, 10.0, 20.0] {
            for &lb in &[3usize, 5, 10] {
                grid.push(RocPctCfg { lp, sp, lb, is_baseline: false });
            }
        }
    }
    grid
...
fn roc(data: &[f64], period: usize) -> Vec<f64> {
    ...
    for i in period..n {
        if data[i-period] != 0.0 {
            r[i] = (data[i] - data[i-period]) / data[i-period] * 100.0;
        }
    }
    r
}

fn percentile_rank(roc_history: &[f64], current: f64) -> f64 {
    if roc_history.is_empty() { return 50.0; }
    let count = roc_history.iter().filter(|&&x| x.abs() >= current.abs()).count();
    count as f64 / roc_history.len() as f64 * 100.0
}
...
fn load_15m(coin: &str) -> Option<CoinData> {
    ...
    // Use lookback=5 as default for data storage
    let roc = roc(&closes, 5);

    Some(CoinData { name: coin.to_string(), opens, closes, highs, lows, zscore, roc })
}
...
fn simulate(d: &CoinData, cfg: &RocPctCfg) -> CoinResult {
    let n = d.closes.len();
    ...
    // Rolling ROC history
    let mut roc_history: Vec<f64> = Vec::with_capacity(100);
    let window = 100usize;

    // Precompute ROC for this lookback
    let roc_vals = roc(&d.closes, cfg.lb);

    for i in 20..n {
        cooldown = cooldown.saturating_sub(1);

        let roc_val = roc_vals[i];
        if !roc_val.is_nan() {
            roc_history.push(roc_val);
            if roc_history.len() > window {
                roc_history.remove(0);
            }
        }
        ...
        if let Some((dir, entry, entry_bar)) = pos.as_mut() {
            ...
            if let Some(dir) = regime_signal(d.zscore[i]) {
                if cfg.is_baseline {
                    ...
                    let passes_filter = if !roc_val.is_nan() && roc_history.len() >= 20 {
                        let pct_rank = percentile_rank(&roc_history, roc_val);
                        if dir == 1 {
                            pct_rank <= cfg.lp
                        } else {
                            pct_rank <= cfg.sp
...
pub fn run(shutdown: Arc<AtomicBool>) {
    eprintln!("RUN117 — ROC Percentile Filter Grid Search\n");
    ...
    println!("\n=== RUN117 ROC Percentile Filter Results ===");
    ...
    let notes = format!("RUN117 ROC percentile filter. {} configs. Baseline PnL={:.2}. Best: {} (PnL={:.2}, Δ={:+.2})",
    ...
    std::fs::write("/home/scamarena/ProjectCoin/run117_1_results.json", &serde_json::to_string_pretty(&output).unwrap()).ok();
    eprintln!("\nSaved → run117_1_results.json");
}
