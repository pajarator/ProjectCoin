/// RUN127 — Force Index Confirmation: Elder's Force Index for Momentum Confirmation
///
/// Grid: FORCE_EMA [7, 13, 21] × LONG_MIN [-1M, 0, 1M] × SHORT_MAX [-1M, 0, 1M]
/// 27 configs × 18 coins = 486 simulations (parallel per coin)
///
/// Run: cargo run --release --features run127 -- --run127
...
impl ForceCfg {
    fn label(&self) -> String {
        if self.is_baseline { "BASELINE".to_string() }
            ...
            let lm = if self.long_min < -999000.0 { "LM".to_string() } else { format!("{:+}", self.long_min as i64) };
            let sm = if self.short_max > 999000.0 { "SM".to_string() } else { format!("{:+}", self.short_max as i64) };
            format!("EP{}_{}_{}", self.ema_period, lm, sm)
        }
    }
}

fn build_grid() -> Vec<ForceCfg> {
    let mut grid = vec![ForceCfg { ema_period: 0, long_min: 0.0, short_max: 0.0, is_baseline: true }];
    for &ep in &[7usize, 13, 21] {
        for &lm in &[-1000000.0f64, 0.0, 1000000.0] {
            for &sm in &[-1000000.0f64, 0.0, 1000000.0] {
                grid.push(ForceCfg { ema_period: ep, long_min: lm, short_max: sm, is_baseline: false });
            }
        }
    }
    grid
...
fn load_15m(coin: &str) -> Option<CoinData> {
    ...
    // Compute Force Index: (close - prior_close) × volume, smoothed with EMA(13)
    let ema_period = 13;
    let alpha = 2.0 / (ema_period as f64 + 1.0);
    let mut force_raw = vec![f64::NAN; n];
    let mut force_ema = vec![f64::NAN; n];
    for i in 1..n {
        force_raw[i] = (closes[i] - closes[i-1]) * volumes[i];
    }
    if force_raw.len() > ema_period {
        force_ema[ema_period] = force_raw[1..=ema_period].iter().sum::<f64>() / ema_period as f64;
        let mut ema_val = force_ema[ema_period];
        for i in ema_period..n {
            ema_val = alpha * force_raw[i] + (1.0 - alpha) * ema_val;
            force_ema[i] = ema_val;
        }
    }

    Some(CoinData { name: coin.to_string(), opens, closes, highs, lows, volumes, zscore, force: force_ema })
}

fn simulate(d: &CoinData, cfg: &ForceCfg) -> CoinResult {
    let n = d.closes.len();
        ...
        if let Some((dir, entry, entry_bar)) = pos.as_mut() {
            ...
            if let Some(dir) = regime_signal(d.zscore[i]) {
                if cfg.is_baseline {
                    ...
                    let force_val = d.force[i];
                    let passes_filter = if force_val.is_nan() {
                        true
                    } else if dir == 1 {
                        // LONG: force must be >= long_min
                        force_val >= cfg.long_min
                    } else {
                        // SHORT: force must be <= short_max
                        force_val <= cfg.short_max
                    };
...
pub fn run(shutdown: Arc<AtomicBool>) {
    eprintln!("RUN127 — Force Index Confirmation Grid Search\n");
    ...
    println!("\n=== RUN127 Force Index Confirmation Results ===");
    ...
    let notes = format!("RUN127 Force Index filter. {} configs. Baseline PnL={:.2}. Best: {} (PnL={:.2}, Δ={:+.2})",
    ...
    std::fs::write("/home/scamarena/ProjectCoin/run127_1_results.json", &serde_json::to_string_pretty(&output).unwrap()).ok();
    eprintln!("\nSaved → run127_1_results.json");
}
