...
///
/// Grid: CCI_LONG_MAX [-150, -100, -50] × CCI_SHORT_MIN [50, 100, 150]
/// 9 configs × 18 coins = 162 simulations (parallel per coin)
///
/// Run: cargo run --release --features run113 -- --run113
...
impl CciCfg {
    fn label(&self) -> String {
        ...
        else { format!("CL{}_CS{}", self.cci_long_max as i32, self.cci_short_min as i32) }
    }
}

fn build_grid() -> Vec<CciCfg> {
    let mut grid = vec![CciCfg { cci_long_max: 0.0, cci_short_min: 0.0, is_baseline: true }];
    for &cl in &[-150.0, -100.0, -50.0] {
        for &cs in &[50.0, 100.0, 150.0] {
            grid.push(CciCfg { cci_long_max: cl, cci_short_min: cs, is_baseline: false });
        }
    }
    grid
}
...
fn load_15m(coin: &str) -> Option<CoinData> {
    ...
    let cci = compute_cci(&highs, &lows, &closes, 20);

    Some(CoinData { name: coin.to_string(), opens, closes, highs, lows, zscore, cci })
}
...
fn simulate(d: &CoinData, cfg: &CciCfg) -> CoinResult {
    let n = d.closes.len();
        ...
        if let Some((dir, entry, entry_bar)) = pos.as_mut() {
            ...
            if let Some(dir) = regime_signal(d.zscore[i]) {
                if cfg.is_baseline {
                    ...
                    let cci_val = d.cci[i];
                    let passes_filter = if !cci_val.is_nan() {
                        if dir == 1 { cci_val < cfg.cci_long_max }
                        else { cci_val > cfg.cci_short_min }
                    } else { true };
...
pub fn run(shutdown: Arc<AtomicBool>) {
    eprintln!("RUN113 — CCI Regime Confirmation Grid Search\n");
    ...
    println!("\n=== RUN113 CCI Regime Confirmation Results ===");
    ...
    let notes = format!("RUN113 CCI confirmation. {} configs. Baseline PnL={:.2}. Best: {} (PnL={:.2}, Δ={:+.2}, filter_rate={:.1}%)",
    ...
    std::fs::write("/home/scamarena/ProjectCoin/run113_1_results.json", &serde_json::to_string_pretty(&output).unwrap()).ok();
    eprintln!("\nSaved → run113_1_results.json");
}
