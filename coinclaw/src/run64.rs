/// RUN64 — Portfolio Position Density Filter
///
/// Grid: MAX_DENSITY [0.50, 0.60, 0.70, 0.80] × DENSITY_COOLDOWN [4, 8, 12]
/// Total: 12 configs + baseline = 13
///
/// Run: cargo run --release --features run64 -- --run64

use rayon::prelude::*;
use serde::Serialize;
use std::sync::atomic::{AtomicBool, AtomicUsize, Ordering};
use std::sync::Arc;

const INITIAL_BAL: f64 = 100.0;
const SL_PCT: f64 = 0.003;
const COOLDOWN: usize = 2;
const BASE_POSITION_SIZE: f64 = 0.02;
const LEVERAGE: f64 = 5.0;

const N_COINS: usize = 18;
const COIN_NAMES: [&str; N_COINS] = [
    "DASH","UNI","NEAR","ADA","LTC","SHIB","LINK","ETH",
    "DOT","XRP","ATOM","SOL","DOGE","XLM","AVAX","ALGO","BNB","BTC",
];

#[derive(Clone, Copy, PartialEq, Debug)]
struct DensityCfg {
    max_density: f64, // 0.0 = disabled (baseline)
    density_cooldown: usize,
}

impl DensityCfg {
    fn label(&self) -> String {
        if self.max_density == 0.0 { "DISABLED".to_string() }
        else { format!("MD{:.2}_DC{}", self.max_density, self.density_cooldown) }
    }
}

fn build_grid() -> Vec<DensityCfg> {
    let mut grid = vec![DensityCfg { max_density: 0.0, density_cooldown: 0 }]; // baseline
    let densities = [0.50, 0.60, 0.70, 0.80];
    let cooldowns = [4, 8, 12];
    for &md in &densities {
        for &dc in &cooldowns {
            grid.push(DensityCfg { max_density: md, density_cooldown: dc });
        }
    }
    grid
}

struct CoinData15m { name: String, closes: Vec<f64>, opens: Vec<f64>, zscore: Vec<f64> }
#[derive(Serialize)] struct ConfigResult { label: String, total_pnl: f64, portfolio_wr: f64, total_trades: usize, pf: f64, is_baseline: bool }
#[derive(Serialize)] struct Output { notes: String, configs: Vec<ConfigResult> }

fn load_15m(coin: &str) -> Option<CoinData15m> {
    let path = format!("/home/scamarena/ProjectCoin/data_cache/{}_USDT_15m_5months.csv", coin);
    let data = std::fs::read_to_string(&path).ok()?;
    let mut opens = Vec::new(); let mut closes = Vec::new();
    for line in data.lines().skip(1) {
        let mut it = line.splitn(7, ','); let _ts = it.next()?;
        let oo: f64 = it.next()?.parse().ok()?; let _hh: f64 = it.next()?.parse().ok()?;
        let _ll: f64 = it.next()?.parse().ok()?; let cc: f64 = it.next()?.parse().ok()?;
        let _vv: f64 = it.next()?.parse().ok()?;
        if oo.is_nan() || cc.is_nan() { continue; }
        opens.push(oo); closes.push(cc);
    }
    if closes.len() < 50 { return None; }
    let n = closes.len();
    let mut zscore = vec![f64::NAN; n];
    for i in 20..n {
        let window = &closes[i+1-20..=i];
        let mean = window.iter().sum::<f64>()/20.0;
        let std = (window.iter().map(|x|(x-mean).powi(2)).sum::<f64>()/20.0).sqrt();
        zscore[i] = if std > 0.0 { (closes[i] - mean) / std } else { 0.0 };
    }
    Some(CoinData15m { name: coin.to_string(), closes, opens, zscore })
}

fn regime_signal(d: &CoinData15m, i: usize) -> Option<i8> {
    if i < 50 { return None; }
    let z = d.zscore[i];
    if z.is_nan() { return None; }
    if z < -2.0 { return Some(1); }
    if z > 2.0 { return Some(-1); }
    None
}

fn simulate_portfolio(coin_data: &[CoinData15m], cfg: DensityCfg) -> (f64, usize, usize, usize) {
    // Run all coins bar-by-bar in sync to properly track portfolio density
    let n = coin_data[0].closes.len();
    let mut bal = INITIAL_BAL;
    let mut pos: Vec<Option<(i8, f64)>> = vec![None; N_COINS];
    let mut cooldown: Vec<usize> = vec![0; N_COINS];
    let mut density_cooldown = 0usize;
    let mut wins = 0usize; let mut losses = 0usize; let mut flats = 0usize;

    for i in 1..n {
        // Count open positions
        let open_count = pos.iter().filter(|p| p.is_some()).count();

        // Process closes
        for ci in 0..N_COINS {
            if let Some((dir, entry)) = pos[ci].take() {
                let d = &coin_data[ci];
                let pct = if dir == 1 { (d.closes[i]-entry)/entry } else { (entry-d.closes[i])/entry };
                let mut closed = false; let mut exit_pct = 0.0;
                if pct <= -SL_PCT { exit_pct = -SL_PCT; closed = true; }
                if !closed {
                    let new_dir = regime_signal(d, i);
                    if new_dir.is_some() && new_dir != Some(dir) { exit_pct = pct; closed = true; }
                }
                if !closed && i >= 2 { exit_pct = pct; closed = true; }
                if closed {
                    let net = (bal * BASE_POSITION_SIZE * LEVERAGE) * exit_pct;
                    bal += net;
                    if net > 1e-10 { wins += 1; } else if net < -1e-10 { losses += 1; } else { flats += 1; }
                    cooldown[ci] = COOLDOWN;
                } else {
                    pos[ci] = Some((dir, entry)); // keep position
                }
            }
        }

        // Decrement cooldowns
        for ci in 0..N_COINS {
            if cooldown[ci] > 0 { cooldown[ci] -= 1; }
        }
        if density_cooldown > 0 { density_cooldown -= 1; }

        // Process new entries
        let current_open = pos.iter().filter(|p| p.is_some()).count();
        let density = current_open as f64 / N_COINS as f64;
        let density_triggered = cfg.max_density > 0.0 && density >= cfg.max_density && density_cooldown == 0;

        if !density_triggered {
            for ci in 0..N_COINS {
                if pos[ci].is_none() && cooldown[ci] == 0 {
                    let d = &coin_data[ci];
                    if let Some(dir) = regime_signal(d, i) {
                        if i+1 < n {
                            let entry_price = d.opens[i+1];
                            if entry_price > 0.0 { pos[ci] = Some((dir, entry_price)); }
                        }
                    }
                }
            }
        } else {
            // Trigger density cooldown
            density_cooldown = cfg.density_cooldown;
        }
    }

    (bal - INITIAL_BAL, wins, losses, flats)
}

pub fn run(shutdown: Arc<AtomicBool>) {
    eprintln!("RUN64 — Portfolio Position Density Filter\n");
    eprintln!("Loading 15m data for {} coins...", N_COINS);
    let mut raw_data: Vec<Option<CoinData15m>> = Vec::new();
    for &name in &COIN_NAMES {
        let loaded = load_15m(name);
        if let Some(ref data) = loaded { eprintln!("  {} — {} bars", name, data.closes.len()); }
        raw_data.push(loaded);
    }
    if !raw_data.iter().all(|r| r.is_some()) { eprintln!("Missing data!"); return; }
    if shutdown.load(Ordering::SeqCst) { return; }
    let coin_data: Vec<CoinData15m> = raw_data.into_iter().map(|r| r.unwrap()).collect();
    let grid = build_grid();
    eprintln!("\nGrid: {} configs × {} coins (portfolio-level)", grid.len(), N_COINS);

    let done = AtomicUsize::new(0);
    let total_cfgs = grid.len();

    let results: Vec<ConfigResult> = grid.par_iter().map(|cfg| {
        if shutdown.load(Ordering::SeqCst) {
            return ConfigResult { label: cfg.label(), total_pnl: 0.0, portfolio_wr: 0.0, total_trades: 0, pf: 0.0, is_baseline: cfg.max_density == 0.0 };
        }
        let (pnl, wins, losses, flats) = simulate_portfolio(&coin_data, *cfg);
        let total_trades = wins + losses + flats;
        let portfolio_wr = if total_trades > 0 { wins as f64 / total_trades as f64 * 100.0 } else { 0.0 };
        let is_baseline = cfg.max_density == 0.0;
        let d = done.fetch_add(1, Ordering::SeqCst) + 1;
        eprintln!("  [{:>2}/{}] {}  PnL={:>+7.2}  WR={:>5.1}%  trades={}", d, total_cfgs, cfg.label(), pnl, portfolio_wr, total_trades);
        ConfigResult { label: cfg.label(), total_pnl: pnl, portfolio_wr, total_trades, pf: 0.0, is_baseline }
    }).collect();

    if shutdown.load(Ordering::SeqCst) { return; }

    let baseline = results.iter().find(|r| r.is_baseline).unwrap();
    let mut sorted: Vec<&ConfigResult> = results.iter().collect();
    sorted.sort_by(|a,b| b.total_pnl.partial_cmp(&a.total_pnl).unwrap_or(std::cmp::Ordering::Equal));

    println!("\n=== RUN64 Portfolio Density Filter Results ===");
    println!("Baseline: PnL={:+.2}  WR={:.1}%  Trades={}", baseline.total_pnl, baseline.portfolio_wr, baseline.total_trades);
    println!("\n{:>3}  {:<14} {:>8} {:>8} {:>8} {:>8}", "#", "Config", "PnL", "ΔPnL", "WR%", "Trades");
    println!("{}", "-".repeat(55));
    for (i,r) in sorted.iter().enumerate() {
        let delta = r.total_pnl - baseline.total_pnl;
        println!("{:>3}  {:<14} {:>+8.2} {:>+8.2} {:>6.1}% {:>6}", i+1, r.label, r.total_pnl, delta, r.portfolio_wr, r.total_trades);
    }
    println!("{}", "=".repeat(55));

    let best = sorted.first().unwrap();
    let is_positive = best.total_pnl > baseline.total_pnl;
    println!("\nVERDICT: {}", if is_positive { "POSITIVE" } else { "NEGATIVE" });

    let notes = format!("RUN64 portfolio density filter. {} configs. Baseline PnL={:.2}. Best: {} (PnL={:.2}, Δ={:+.2})",
        results.len(), baseline.total_pnl, best.label, best.total_pnl, best.total_pnl - baseline.total_pnl);
    let output = Output { notes, configs: results };
    std::fs::write("/home/scamarena/ProjectCoin/run64_1_results.json", &serde_json::to_string_pretty(&output).unwrap()).ok();
    eprintln!("\nSaved → run64_1_results.json");
}
