#!/usr/bin/env python3
"""
COINCLAW v6 - Long+Short Directional Trading
Detects market conditions per coin, trades long when breadth low, short when breadth high.
"""
import ccxt
import pandas as pd
import time
...
# === COINS (RUN4.2 optimal long strategy + RUN5.2 short strategy per coin) ===
COINS = [
    {'symbol': 'DASH/USDT',  'tf': '15m', 'name': 'DASH',  'pref': 'vwap_rev',  'short_pref': 'short_mean_rev'},    # Short PF=7.33
    {'symbol': 'UNI/USDT',   'tf': '15m', 'name': 'UNI',   'pref': 'vwap_rev',  'short_pref': 'short_adr_rev'},    # Short PF=2.82
    {'symbol': 'NEAR/USDT',  'tf': '15m', 'name': 'NEAR',  'pref': 'vwap_rev',  'short_pref': 'short_adr_rev'},    # Short PF=3.38
    {'symbol': 'ADA/USDT',   'tf': '15m', 'name': 'ADA',   'pref': 'vwap_rev',  'short_pref': 'short_bb_bounce'},  # Short PF=6.46
    {'symbol': 'LTC/USDT',   'tf': '15m', 'name': 'LTC',   'pref': 'vwap_rev',  'short_pref': 'short_mean_rev'},   # Short PF=9.12
    {'symbol': 'SHIB/USDT',  'tf': '15m', 'name': 'SHIB',  'pref': 'vwap_rev',  'short_pref': 'short_vwap_rev'},   # Short PF=3.64
    {'symbol': 'LINK/USDT',  'tf': '15m', 'name': 'LINK',  'pref': 'vwap_rev',  'short_pref': 'short_bb_bounce'},  # Short PF=4.60
    {'symbol': 'ETH/USDT',   'tf': '15m', 'name': 'ETH',   'pref': 'vwap_rev',  'short_pref': 'short_adr_rev'},    # Short PF=3.11
    {'symbol': 'DOT/USDT',   'tf': '15m', 'name': 'DOT',   'pref': 'vwap_rev',  'short_pref': 'short_vwap_rev'},   # Short PF=11.61
    {'symbol': 'XRP/USDT',   'tf': '15m', 'name': 'XRP',   'pref': 'vwap_rev',  'short_pref': 'short_bb_bounce'},  # Short PF=2.69
    {'symbol': 'ATOM/USDT',  'tf': '15m', 'name': 'ATOM',  'pref': 'vwap_rev',  'short_pref': 'short_adr_rev'},    # Short PF=2.92
    {'symbol': 'SOL/USDT',   'tf': '15m', 'name': 'SOL',   'pref': 'vwap_rev',  'short_pref': 'short_adr_rev'},    # Short PF=4.02
    {'symbol': 'DOGE/USDT',  'tf': '15m', 'name': 'DOGE',  'pref': 'bb_bounce', 'short_pref': 'short_bb_bounce'},  # Short PF=1.55
    {'symbol': 'XLM/USDT',   'tf': '15m', 'name': 'XLM',   'pref': 'dual_rsi',  'short_pref': 'short_mean_rev'},   # Short PF=15.94
    {'symbol': 'AVAX/USDT',  'tf': '15m', 'name': 'AVAX',  'pref': 'adr_rev',   'short_pref': 'short_bb_bounce'},  # Short PF=7.70
    {'symbol': 'ALGO/USDT',  'tf': '15m', 'name': 'ALGO',  'pref': 'adr_rev',   'short_pref': 'short_adr_rev'},    # Short PF=14.48
    {'symbol': 'BNB/USDT',   'tf': '15m', 'name': 'BNB',   'pref': 'vwap_rev',  'short_pref': 'short_vwap_rev'},   # Short PF=5.32
    {'symbol': 'BTC/USDT',   'tf': '15m', 'name': 'BTC',   'pref': 'bb_bounce', 'short_pref': 'short_adr_rev'},    # Short PF=4.54
]
...
class Trader:
    def __init__(self, s, tf, name, pref, short_pref):
        self.sym = s
        self.tf = tf
        self.name = name
...
traders = [Trader(c['symbol'], c['tf'], c['name'], c['pref'], c['short_pref']) for c in COINS]
...
def main(stdscr):
    ...
    while True:
        ...
        stdscr.addstr(1, 0, f" COINCLAW v6 - {len(COINS)} COINS | {len(COINS)}x${INITIAL_CAPITAL} = ${len(COINS)*INITIAL_CAPITAL}", curses.color_pair(1) | curses.A_BOLD)
        stdscr.addstr(2, 0, f" Risk:{RISK*100:.0f}% | {LEVERAGE}x LEV | SL:{STOP_LOSS*100:.1f}% | Hold:{MIN_HOLD_CANDLES}candles | Long<={BREADTH_MAX*100:.0f}% Short>={SHORT_BREADTH_MIN*100:.0f}% | {datetime.now().strftime('%H:%M:%S')}")
        ...
        if market_mode == 'LONG':
            breadth_str = f" BREADTH: {breadth_pct:.0f}% ({bearish_count}/{len(z_scores) if z_scores else 0}) - MODE: LONG (entries OK)"
            stdscr.addstr(y, 0, breadth_str, curses.color_pair(3))
        elif market_mode == 'SHORT':
            breadth_str = f" BREADTH: {breadth_pct:.0f}% ({bearish_count}/{len(z_scores) if z_scores else 0}) - MODE: SHORT (shorting overbought)"
            stdscr.addstr(y, 0, breadth_str, curses.color_pair(6))
        else:
            breadth_str = f" BREADTH: {breadth_pct:.0f}% ({bearish_count}/{len(z_scores) if z_scores else 0}) - MODE: IDLE (no new entries)"
        ...
        for idx, t in enumerate(traders):
            ...
            t.active_strat = long_strat if market_mode == 'LONG' else (short_strat if market_mode == 'SHORT' else None)
            ...
            # Process entries based on directional mode
            if not t.pos:
                if market_mode == 'LONG' and long_strat is not None and t.entry(i, long_strat):
                    t.open_position(i['p'], regime, long_strat, i, direction='long')
                elif market_mode == 'SHORT' and short_strat is not None and t.short_entry(i, short_strat):
